<template>
	<view class="tree-item" :class="{'treeView':item.pid==0}">
        <view class="head h-flex">
            <view class="head-1 flex-1" @click="changeShow" @longpress="longPress" >
               
                <text class="txt">{{item[defaultProps.label]}}</text>
				<image src="../../../static/down.png" :class="show ? 'rt45' : ''" mode="scaleToFill" class="left-icon"
				    v-if="item[defaultProps.children] && item[defaultProps.children].length > 0"></image>
				<uni-icons v-if="is_choice&&item.id==choice_id"
						type='checkmarkempty' size="24" color="#157CF2"></uni-icons>
            </view>
		...
		methods: {
			changeShow() {
				if (this.item[this.defaultProps.children] && this.item[this.defaultProps.children].length > 0) {
					...
					if(this.choice_id == this.item.id && this.is_clean){
						this.choice_id = ''
					}else this.choice_id = this.item.id
				}
