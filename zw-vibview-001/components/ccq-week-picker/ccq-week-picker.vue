<template>
	...
	<view class="ccq-week-picker">
		...
		<picker-view :value="pickerValue" @change="bindChange" class="ccq-week-picker_view"
			...
			<picker-view-column v-if="fields=='week'" class="ccq-week-picker_item weeks">
	...
	export default {
		...
		methods: {
			async dateInit() {
				...
				let s = Number(dayjs().year()) - 50;
				let e = Number(dayjs().year());
				let y = Number(v[0]),
					y_index;
				let m = Number(v[1]);
				// 年
...
<style lang="scss" scoped>
	...
	.ccq-week-popup_header {
		...
		align-items: center;
		background-color: #FFFFFF;
		border-bottom: 1rpx solid #EBEDF1;
		border-radius: 30rpx 30rpx 0 0;
	}
