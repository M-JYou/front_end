	...
	<view class="pages">
		...
		<view class="content">
			...
			<view class="charts-box">
				<qiun-data-charts class="chartsed" type="line" :opts="opts" :chartData="chartData" :ontouch="true"
			...
			return {
				...
				opts: {
					...
					xAxis: {
						...
						scrollShow: false,
						itemCount:10000,
						scrollBackgroundColor: '#A6A6A6', //默认为 #EFEBEF
						scrollColor: '#A6A6A6', //默认为 #A6A6A6
						dashLength:3,
						scrollAlign:'left',
						scrollWidth: 100, //滚动条初始位置
		...
		methods: {
			...
			async getitemsList(pointcode) {
				...
				if (res.statusCode == 200) {
					...
					this.value = res.data[1].name
					console.log(this.value, "选择")
				}
			},
			//获取曲线数据
			gatDatalist(codes) {
				console.log(this.signals, 'signal')
				let params = {
					Origin: "app",
					RequestData: {
						// pointId:"10070010010301",
						pointId: codes,
						signalType: this.signals,
						startTime: this.stime,
						endTime: this.etime
					},
				}
				// console.log(params.RequestData.signalType, "得到code")
				gatcurveDatalist(JSON.stringify(params)).then(res => {
					...
					if (res.statusCode == 200) {
						...
						console.log(this.unitsed,10)
						this.units=res.data.engineerUnit
					if(this.signals==100){
						this.units="℃"
					}
			...
			getServerData() {
				...
				setTimeout(() => {
					...
					console.log(this.chartData,5425)
				}, 500);
			},
			//跳时频谱
			goFrequency() {
