...
<script>
	...
	export default {
		...
		onLoad(options) {
			...
			// console.log(this.signal, "12121")
			this.getWavedata(this.pointId, this.signal, this.dataValue, this.dataIndex);
		...
		methods: {
			...
			//获取上一刻数据
			getWavedata(pointId, signal, dataIndex, dataValue) {
				let params = {
					Origin: "app",
					RequestData: {
						pointId: pointId,
				...
				gatWavedata(JSON.stringify(params)).then(res => {
					...
					if (res.statusCode == 200) {
						...
						this.engineerUnited = res.data.engineerUnit //单位值
						// console.log(this.engineerUnited,122)
