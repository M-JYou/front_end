<template>
  <view class="container">
    <!-- 列表 -->
    <view v-for="(item, index) in purchaseData" :key="index">
      ...
      <view v-else>
        ...
        <view class="order-item">
          <view class="order-content">
            <view class="">
              <text class="order-name">是否急需</text>
            </view>
            <view class="price">
              <text style="font-size: 14rpx;">{{isBadlyNeedStatus[item.isBadlyNeed]}}</text>
            </view>
          </view>
        </view>
...
<script>
  export default {
    ...
    onLoad(options) {
      ...
      this.purchaseData = [JSON.parse(options.item2)]
    }
  };
</script>
