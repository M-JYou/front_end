<template>
  <view class="hrListItem">
    ...
    <view class="wrapbox" v-for="(item,i) in cooperativeData" :key="item.id">
      <view v-if="item.name==''">暂时没有数据</view>
      <view class="wrap" @click="handleToDetails(item)" v-else>
        <text class="wrapLeft">协作单位名称：{{item.name}}</text>
  ...
  export default {
    data() {
      return {
        ...
        cooperativeData: [],
        URL: '/pages/work/hr/cooperative/indexDetails'
      }
    },

    methods: {
      getData() {
        getCooperative(this.formData).then(res => {
          ...
          if (res.code == 200) {
            ...
            this.cooperativeData = [...this.cooperativeData, ...newList]
            // 关闭提示框
            setTimeout(function() {
              uni.hideLoading();
            }, 1000);
