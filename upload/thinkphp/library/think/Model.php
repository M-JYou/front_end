...
abstract class Model implements \JsonSerializable, \ArrayAccess {
  ...
  public function __construct($data = []) {
    ...
    if (is_null($this->autoWriteTimestamp)) {
      // 自动写入时间戳
      $this->autoWriteTimestamp = $this->getQuery()->getConfig(
        'auto_timestamp'
      );
    }
    ...
    if (is_null($this->resultSetType)) {
      $this->resultSetType = $this->getQuery()->getConfig(
        'resultset_type'
      );
    }
    // 执行初始化操作
  ...
  protected function buildQuery() {
    ...
    if (
      isset(static::$readMaster['*']) ||
      isset(static::$readMaster[$this->class])
    ) {
      $query->master(true);
    }
  ...
  public function getData($name = null) {
    if (is_null($name)) {
      ...
      throw new InvalidArgumentException(
        'property not exists:' . $this->class . '->' . $name
      );
    }
  }
  ...
  public function setAttr($name, $value, $data = []) {
    if (
      ...
      in_array($name, [$this->createTime, $this->updateTime])
    ) {
      // 自动写入的时间戳字段
  ...
  protected function autoWriteTimestamp($name) {
    if (isset($this->type[$name])) {
      ...
      is_string($this->autoWriteTimestamp) &&
      in_array(strtolower($this->autoWriteTimestamp), [
        'datetime',
        'date',
        'timestamp'
      ])
  ...
  public function hidden($hidden = [], $override = false) {
    $this->hidden = $override
      ? $hidden
      : array_merge($this->hidden, $hidden);
    return $this;
  }
  ...
  public function visible($visible = [], $override = false) {
    $this->visible = $override
      ? $visible
      : array_merge($this->visible, $visible);
    return $this;
  }
  ...
  public function save($data = [], $where = [], $sequence = null) {
    ...
    if ($this->isUpdate) {
      ...
      if (
        empty($data) ||
        (count($data) == 1 && is_string($pk) && isset($data[$pk]))
      ) {
        // 关联更新
        if (isset($relation)) {
      ...
      $allowFields = $this->checkAllowField(
        array_merge($this->auto, $this->update)
      );
      ...
      if ($this->autoWriteTimestamp) {
        if (
          $this->createTime &&
          !isset($this->data[$this->createTime])
        ) {
      ...
      // 检测字段
      $allowFields = $this->checkAllowField(
        array_merge($this->auto, $this->insert)
      );
      if (!empty($allowFields)) {
        $result = $this->getQuery()
          ->strict(false)
  ...
  public function getChangedData() {
    if ($this->force) {
      ...
      $data = array_udiff_assoc($this->data, $this->origin, function (
        $a,
        $b
      ) {
  ...
  public function relationQuery($relations) {
    ...
    foreach ($relations as $key => $relation) {
      ...
      $this->data[$relation] = $this->$method()->getRelation(
        $subRelation,
        $closure
      );
    }
  ...
  public function eagerlyResultSet(&$resultSet, $relation) {
    ...
    foreach ($relations as $key => $relation) {
      ...
      $relation = Loader::parseName($relation, 1, false);
      $this->$relation()->eagerlyResultSet(
        $resultSet,
        $relation,
        $subRelation,
        $closure
  ...
  public function __isset($name) {
    try {
      if (
        ...
        array_key_exists($name, $this->relation)
      ) {
        return true;
      } else {
        $this->getAttr($name);
