...
class Job extends \app\common\model\BaseModel {
  ...
  protected function getCompanyAttr($v) {
    $pre = config('database.prefix');
    $t = clo(model('Company')->whereOr('a.id', $v)->whereOr('a.uid', $v)->alias('a')
      ->join(
        $pre . 'company_info b',
        'a.id=b.comid',
        'left'
      )->join(
        $pre . 'company_contact c',
        'a.id=c.comid',
        'left'
      )->join(
        $pre . 'member d',
      ...
      ->field(
        'a.id,a.uid user,companyname,nature,trade,district,scale'
          . ',concat(registered,if(currency,"万美元","万人民币")) registered'
          . ',concat("[",tag,"]") tag,map_lat,map_lng,map_zoom,audit,logo,addtime' .
          ',click,user_status,b.content,b.website,b.address'
          . ',c.contact,c.mobile,c.weixin,c.telephone,c.qq,c.email,d.last_login_time'
      )
      ->find());
    if ($t) {
      $category = model('Category')->column('id,name');
      $t['nature'] = $category[$t['nature']];
      $t['trade'] = $category[$t['trade']];
      $t['scale'] = $category[$t['scale']];
      $t['user_status'] = $t['user_status'] ? '正常' : '异常';
      $st = decode($t['tag']);
      $t['tag'] = [];
      foreach ($st as $k) {
        if (isset($category[$k])) {
      ...
      $t['logo'] = model('Uploadfile')->getFileUrl($t['logo']);
      $auth = model('Auth')->getByUid($t['uid']);
      $t['auth'] = [];
      $t['legal_person'] = '未认证法人';
      foreach ($auth as $vv) {
        $t['auth'][] = $vv['type'];
        if ($vv['type'] == '企业认证') {
          $t['legal_person'] = decode($vv)[4]['url'];
        }
      }
      $t['jobsum'] = model('Job')->where('company_id', $t['id'])->count();
      $s = intval((time() - $t['last_login_time']) / 60);
      ...
      $t['hy'] = ($s > 1440
        ...
        : ($s > 60
          ...
          : ($s . '分钟')))
        . '前活跃';
      $t['addtime'] = date('Y-m-d', $t['addtime']);
      $t['applysum'] = model('JobApply')->where('comid', $t['id'])->count();
    }
    return $t;
  }

  protected function getTag_Attr($v) {
    return model('Category')->where('id', 'in', parseArr($v))->column('name');
  }
  protected function getApplyTextAttr($v, $d) {
    return model('JobApply')->where(['personal_uid' => $v, 'jobid' => $d['id']])->field('id')->find()
      ? '已投递' : '投递';
  }
