...
class Order extends \app\common\model\BaseModel {
  ...
  public function getOtherAttr($v, $d = []) {
    $ret = [];
    switch ($v) {
      case 'goods':
        $goods = model('Goods')->find($d['service_id']);
        $store = null;
        $mmi = model('MemberInfo');
        if ($goods) {
          $store = $mmi->field('id,name,photo')->find($goods['create']);
        }
        $ret['shop'] = $store ? $store :
  ...
  public function addSetmealOrder($data) {
    $timestamp = time();
    $service_info = model('Setmeal')->where('id', 'eq', $data['service_id'])->find();
    ...
    if ($data['coupon_id'] > 0) {
      ...
      $coupon_info = model('CouponRecord')->where($coupon_map)->find();
    }
  ...
  public function addPointsOrder($data) {
    $timestamp = time();
    $service_info = model('CompanyServicePoints')->where('id', 'eq', $data['service_id'])->find();
    if ($service_info === null) {
      $this->error = '未获取到服务信息';
      return false;
    }
  ...
  public function addOrderSingleServiceOrder($data) {
    ...
    if ($data['service_type'] == 'single_resume_down') {
      $resume_info = model('Resume')->field('high_quality,uid,refreshtime,fullname')->where('id', 'eq', $data['resumeid'])->find();
    ...
    if ($data['service_type'] == 'single_resume_down') {
      ...
      $jobinfo = model('Job')->where('id', $data['jobid'])->field('jobname')->find();
      $order_arr['extra'] = [
        'jobid' => $data['jobid'],
  ...
  public function addOrder($data) {
    $data['utype'] = model('Member')->where('uid', $data['uid'])->field('utype')->find()['utype'];
    $addToQueue = 0;
    $setPoints = null;
    switch ($data['service_type']) {
      ...
      case 'goods':
        if (!isset($data['note'])) {
          $this->error = '缺少必要字段note';
          return false;
        }
        $service_model = model('Goods')->field('id,expense,sum,');
        $data['deduct_points'] = 1;
        $setPoints = [
          function (&$param) use (&$data) {
            $data['sum'] = abs($data['sum']);
            if ($data['sum'] > $param['sum']) {
              $data['sum'] = $param['sum'];
            }
            $note = '【' . $param['name'] . '】*' . $data['sum'];
            $expense = ($param['expense'] * $data['sum'] + $param['freight']);
            $r = [['uid' => $data['uid'], 'points' => -1 * $expense, 'note' => '购买' . $note,]];
            if ($param['create']) {
              $r[] = ['uid' => $param['create'], 'points' => $expense, 'note' => '销售' . $note,];
            }
            $param['expense'] = $expense;
            return $r;
          },
          function () use (&$data) {
            $item = model('Goods')->field('id,sum')->find($data['service_id']);
            model('Goods')->update([
              'sum' => ['dec', $data['sum']],
              'sales' => ['inc', $data['sum']],
            ], ['id' => $item['id']]);
      ...
      case 'cart':
        ...
        $service_model = model('Cart')->where('status', 0);
        $data['deduct_points'] = 1;
        $setPoints = [
          function (&$param) use (&$data) {
            $param['name'] = '购买【' . $param['goods'][0]['name'] . '】等' . count($param['goods']) . '类商品。';
            $ret = [];
            $ret[] = [
              'uid' => $data['uid'],
              'points' => -1 * $param['expense'] * config('global_config.payment_rate'),
              'note' => $param['name']
            ];
            foreach ($param['goods'] as $v) {
              $ret[] = [
                'uid' => $v['create'],
                'points' => config('global_config.payment_rate') * ($v['sum'] * $v['expense'] + $v['freight']),
                'note' => '销售【' . $v['name'] . '】' . $v['sum'] . '件商品。',
              ];
            }
            return $ret;
          },
          function () use (&$data) {
            model('Cart')->update(['status' => 1], ['id' => $data['service_id']]);
            $m = model('Goods');
    ...
    $service_info = clo($service_model->where('id', 'eq', $data['service_id'])->find());
    if ($service_info === null) {
      $this->error = '未获取到服务信息';
      return false;
    }
    if (is_array($setPoints) && count($setPoints) && isFunction($setPoints[0])) {
      $service_info['enable_points_deduct'] = 1;
      $setPoints[0] = $setPoints[0]($service_info);
    }

    $order_arr['code'] = isset($data['code']) ? $data['code'] : '';
    $order_arr['openid'] = isset($data['openid']) ? $data['openid'] : '';
    $order_arr['return_url'] = isset($data['return_url']) ? $data['return_url'] : "";
    ...
    if ($data['utype'] == 1) {
      ...
      if ($member_setmeal['service_added_discount'] > 0) {
        $order_arr['service_amount_after_discount'] = ($order_arr['service_amount'] / 10) * $member_setmeal['service_added_discount'];
      } else {
        $order_arr['service_amount_after_discount'] = $order_arr['service_amount'];
      }
      $order_arr['extra']['service_added_discount'] = $member_setmeal['service_added_discount'];
    } else {
      $order_arr['service_amount_after_discount'] = $order_arr['service_amount'];
    }
    //=============================================================
    ...
    $order_arr['deduct_amount'] = 0;
    $order_arr['payment'] = '';
    $order_arr['oid'] = date('Ymd') . msectime() . substr(md5($order_arr['uid']), rand(0, 27), 4);
    if ($order_arr['service_amount_after_discount'] == 0) {
      ...
      if ($service_info['enable_points_deduct'] == 0) {
        ...
        //允许全额抵扣
        $order_arr['amount'] = 0;
        $order_arr['deduct_amount'] = $order_arr['service_amount_after_discount'];
        // 是否有数量字段，是则表示点券
        $order_arr['deduct_points'] = isset($service_info['sum']) ? $order_arr['service_amount_after_discount'] : ceil($order_arr['service_amount_after_discount'] * config('global_config.payment_rate'));
        $data['payment'] = 'points';
      } else {
        //允许部分抵扣
      ...
      if ($order_arr['deduct_points'] > model('Member')->getMemberPoints($order_arr['uid'])) {
    ...
    \think\Db::startTrans();
    try {
      $order_arr['note'] = isset($data['note']) ?
        (is_string($data['note']) ?
          $data['note'] : json_encode($data['note'], JSON_UNESCAPED_UNICODE)
        )
        : '';
      $order_arr['add_platform'] = config('platform');
      $order_arr['pay_platform'] = '';
  ...
  public function orderClose($id, $uid) {
    ...
    if (isset($extra['coupon_info'])) {
      $coupon_info = model('CouponRecord')->where('id', $extra['coupon_info']['id'])->find();
      if ($coupon_info !== null) {
  ...
  /** 设置订单状态为已支付 */
  public function orderPaid($oid, $payment, $time, $note = null, $admin_id = 0) {
    $setPoints = null;
    if (is_array($note) && count($note)) {
      $setPoints = $note;
      $note = null;
      model('MemberPoints')->setPoints($setPoints[0]);
    }
    ...
    if ($order['utype'] == 2 && $order['service_type'] == 'stick') {
      model('Resume')->where('uid', 'eq', $order['uid'])->setField('stick', 1);
      model('ResumeSearchRtime')->where('uid', 'eq', $order['uid'])->setField('stick', 1);
      model('ResumeSearchKey')->where('uid', 'eq', $order['uid'])->setField('stick', 1);
      $addToQueue = 1;
      $points_log = '简历置顶';
    }
    //简历标签
    if ($order['utype'] == 2 && $order['service_type'] == 'tag') {
      $extra = json_decode($order['extra'], true);
      model('Resume')->where('uid', 'eq', $order['uid'])->setField('service_tag', $extra['tag_text']);
      $addToQueue = 1;
      $points_log = '简历标签';
    }
    //职位置顶
    if ($order['utype'] == 1 && $order['service_type'] == 'jobstick') {
      $extra = json_decode($order['extra'], true);
      model('Job')->where('id', 'eq', $extra['jobid'])->setField('stick', 1);
      model('JobSearchRtime')->where('id', 'eq', $extra['jobid'])->setField('stick', 1);
      model('JobSearchKey')->where('id', 'eq', $extra['jobid'])->setField('stick', 1);
      $addToQueue = 1;
      $points_log = '职位置顶';
    }
    //职位紧急
    if ($order['utype'] == 1 && $order['service_type'] == 'emergency') {
      $extra = json_decode($order['extra'], true);
      model('Job')->where('id', 'eq', $extra['jobid'])->setField('emergency', 1);
      model('JobSearchRtime')->where('id', 'eq', $extra['jobid'])->setField('emergency', 1);
      model('JobSearchKey')->where('id', 'eq', $extra['jobid'])->setField('emergency', 1);
      $addToQueue = 1;
      $points_log = '职位紧急';
    }
    //简历包
    if (
      ...
      if ($extra['download_resume_point'] > 0) {
        model('MemberSetmeal')->where('uid', $order['uid'])->setInc(
    ...
    if (!$setPoints && $changePoints && $order['deduct_points'] > 0) {
      model('Member')->setMemberPoints([
        'uid' => $order['uid'],
        'points' => $order['deduct_points'],
        'note' => config('global_config.points_byname') . '抵扣-' . $points_log
      ], 2);
    }
    ...
    if ($order['utype'] == 1) {
      ...
      model('AdminNotice')->send(
        ...
        '订单号:【' . $order['oid'] . '】' . "\r\n" .
          ...
          '服务内容:【' . $order['service_name'] . '】' . "\r\n" .
          '订单金额:【' . $order['amount'] . '】' . "\r\n" .
          '订单状态:【支付成功】',
        '企业订单支付成功通知，请及时跟进（后台->业务->企业业务管理->订单管理）'
      );
    }
    if ($order['utype'] == 2) {
      ...
      model('AdminNotice')->send(
        ...
        '个人订单支付成功通知',
        '订单号:【' . $order['oid'] . '】' . "\r\n" .
          '个人名称:【' . $resume['fullname'] . '】' . "\r\n" .
          '服务内容:【' . $order['service_name'] . '】' . "\r\n" .
          '订单金额:【' . $order['amount'] . '】' . "\r\n" .
          '订单状态:【支付成功】',
        '个人订单支付成功通知，请及时跟进（后台->业务->企业业务管理->订单管理）'
      );
    }
    ...
    if (is_array($setPoints) && isFunction($setPoints[1])) {
      $setPoints[1]();
    }
    return true;
  }
  ...
  public function recordToQueue($order) {
    ...
    if (
      ...
      $resume = model('Resume')->where('uid', $order['uid'])->find();
      $queue['pid'] = $resume['id'];
    }
    if (
