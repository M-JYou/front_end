...
class NotifyRule extends \app\common\model\BaseModel {
    ...
    public function notify(
        ...
        if ($ruleOne['open_push'] == 1 && config('global_config.jpush_appkey') && config('global_config.jpush_appsecret')) {
            $jpush = new \app\common\lib\Jpush();
            $jpush->notification($uid, '【' . model('Message')->map_type[$ruleOne['type']] . '】' . $content, $ruleOne['inner_link'], $link_param);
        }
    }
