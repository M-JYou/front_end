...
class def {
  ...
  public function getByCache($pageCache = null, $pageAlias = null) {
    $cacheName = 'cache_index_json';
    if (false === ($data = cache($cacheName))) {
      $data = $this->getData($pageCache, $pageAlias);
      cache($cacheName, $data, is_array($pageCache) && isset($pageCache['expire']) && $pageCache['expire'] > 0 ? $pageCache['expire'] : 600);
    }
    return $data;
  }
  public function getData($pageCache = null, $pageAlias = null) {
    $return['category_jobs'] = $this->getCategoryJobs(); // 职位分类
    $return['notice_list'] = $this->getNoticeList(2); // 公告列表
    $return['event_list'] = $this->getEventList(); // 动态列表
    $return['tag_list'] = $this->getTagList(); // 职位福利
    $return['new_today_list'] = $this->getNewTodayList(); // 今日更新
    $return['emergency_list'] = $this->getEmergencyList(5); // 突然事件
    $return['hotword_list'] = $this->getHotwordList(15); // 热门关键字
    $return['famous_list'] = $this->getFamousList(15); // 优选职位
    $return['hotjob_list'] = $this->getHotjobList(); // 热门职位
    $return['company_list'] = $this->getCompanyList(); // 企业主页
    $return['resume_list'] = $this->getResumeList(); // 优选人才
    $return['article_list'] = $this->getArticleList(); // 职场资讯
    $return['hrtool_list'] = $this->getHrtoolList(); // hr工具箱
    $return['flink_list'] = $this->getFlinkList(); // 友情链接
    $return['banner_list'] = $this->getBannerList(); // 广告
    $return['link_group'] = $this->linkGroup(); // 链接按照分类
    $ma = model('Article');
    $return['wenku'] = $ma->where('cid', 8)
      ->field('id,cid,title,concat("/article/",id) `link_url`')
      ->order('addtime desc')->limit(3)
      ->select();

    // $return['jobs'] = (new \app\index\controller\Job())->getJobs();
    $return['resume_list'] = array_slice($return['resume_list'], 0, 4); // 简历表
    $return['icon'] = model('Config')->where('name', 'like', 'ico_%')->column('name,value');
    $return['link'] = model('link')->getCache(); // 全部链接

    $return['company_transfer'] = model('CompanyTransfer')
      ->where('is_display', 1)
      ->order('change_time desc,id desc')
      ->field('*,`addr` addr_,`type` type_')
      ->limit(5)
      ->select();
    $return['trademark_transfer'] = model('TrademarkTransfer')
      ->where('is_display', 1)
      ->order('change_time desc,id desc')
      ->field('*,`type` type_')
      ->limit(5)
      ->select();
    if ($pageAlias !== null && is_array($pageCache) && $pageCache['expire'] > 0) {
      model('Page')->writeCacheByAlias($pageAlias, $return, $pageCache['expire']);
    }
    $return['qa'] = model('Qa')->order('addtime desc')
      ->field('id,name,concat("/ask/",id) `link`')
      ->limit(2)
      ->select();
  ...
  protected function getHotjobList() {
    ...
    $list = model('JobSearchRtime')->alias('a')
      ...
      ->limit(3)
      ->column('b.id,b.jobname,b.district,b.negotiable,b.minwage,b.maxwage,b.tag,b.company_id,c.companyname');
    $category_district_data = model('CategoryDistrict')->getCache();
    $category_data = model('Category')->getCache();
      ...
      if ($arr['tag']) {
        ...
        foreach ($tag_arr as $v) {
          if (
            ...
            $arr['tag'][] = $category_data['QS_jobtag'][$v];
          } else {
            $arr['tag'][] = $v;
          }
        }
        $arr['tag'] = join(' ', $arr['tag']);
      } else {
        $arr['tag'] = '';
      }
      $list[$key] = $arr;
  ...
  /** 友情链接 */
  public function getFlinkList($type = 0) {
    // $list = model('Link')->order('sort_id desc')->where('is_display', 1)->where('type', $type)->column('id,name,link_url,link_ico');
    // return $list;
    return model('Link')->getByType($type);
  }
  /** 广告 */
