...
class tpl3 extends def {
  ...
  public function getData($pageCache, $pageAlias) {
    ...
    $return['resume_list'] = $this->getResumeList(8);
    ...
    // 最新职位
    $return['newjob_list'] = $this->getNewjobList(16);
    // 推荐企业
    $return['hot_company_list'] = $this->getHotCompanyList();
    // 名企
    $return['famous_company_list'] = $this->getFamousCompany();
    // 新闻资讯分类
    $return['article_category'] = $this->getArticleCategory(6);
    // 新闻资讯列表
    $return['article_list'] = $this->getArticleListByCategory($return['article_category']);

    // 招聘会
    $return['jobfair_list'] = $this->getJobfairList(3);
  ...
  protected function getEmergencyList($limit = 5) {
    ...
    $list = model('JobSearchRtime')->alias('a')
      ...
      ->order('a.refreshtime desc')
      ->limit($limit)
      ->column('b.id,b.emergency,b.jobname,b.negotiable,b.minwage,b.maxwage,b.company_id,c.companyname,c.district,c.district1,c.district2,c.district3,b.education,b.experience');
  ...
  /** 优选人才 */
  protected function getResumeList($limit = 8) {
    ...
    $list = $list->order('a.refreshtime desc')
      ->limit($limit)
      ->column('b.id,a.refreshtime,b.addtime,b.stick,b.high_quality,b.service_tag,b.photo_img,b.fullname,b.display_name,b.sex,b.birthday,b.education,b.enter_job_time');
    ...
    if (!empty($resume_id_arr)) {
      ...
      foreach ($list as $key => $value) {
        ...
        // 是否是新简历
        $arr['is_new'] = (time() - $arr['addtime']) <= 60 * 60 * 24 ? 1 : 0;
        $arr['is_today'] = (time() - $arr['refreshtime']) <= 60 * 60 * 24 ? 1 : 0;
        $arr['is_week'] = (time() - $arr['refreshtime']) <= 60 * 60 * 24 * 7 ? 1 : 0;
   ...
   */
  protected function getHotCompanyList($id = 0, $limit = 5) {
    $returnlist = [];
    $famous_enterprises_setmeal = config(
      'global_config.famous_enterprises'
    );
    ...
    $list = $list->where('s.setmeal_id', 'in', $famous_enterprises_setmeal)
      ->field('c.id,c.logo,c.companyname,c.nature,c.trade,c.district,c.scale')
    ...
    foreach ($list as $key => $value) {
      ...
      $arr['scale_text'] = isset(
        $category_data['QS_scale'][$arr['scale']]
      )
        ? $category_data['QS_scale'][$arr['scale']]
        : '';
  ...
  protected function getFamousCompany($limit = 12) {
    $famous_enterprises_setmeal = config(
      'global_config.famous_enterprises'
    );
    $famous_enterprises_setmeal =
  ...
  protected function getArticleListByCategory($category = []) {
    ...
    foreach ($category as $cid => $cname) {
      $list = model('Article')
        ...
        ->join(config('database.prefix') . 'article_category b', 'a.cid=b.id', 'LEFT')
        ->where('a.is_display', 1)
        ->where('a.cid', $cname['id'])
        ->limit(14) //模板三的职场资讯显示有问题，正常应该是显示14条，实际只能显示13条
        ->order('a.sort_id desc,a.id desc')
        ->column('a.id,a.title,a.link_url,a.addtime,a.cid,b.name as cname');
      foreach ($list as $key => $value) {
        $arr = $value;
