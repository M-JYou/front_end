  ...
  public function alipayNotify($data) {
    ...
    if ($result) {
      ...
      if (
        ...
        if (substr($out_trade_no, 0, 2) == FreelanceOrder::PREFIX_ORDER) {
          $ptOrder = new FreelanceOrder();
          $order = $ptOrder->where('order_sn', $out_trade_no)->find();
          if ($order) {
            return $ptOrder->paid($order, 'alipay', $total_fee, $data['trade_no']);
          }
        }
        if (substr($out_trade_no, 0, 2) == CityinfoOrder::PREFIX_ORDER) {
