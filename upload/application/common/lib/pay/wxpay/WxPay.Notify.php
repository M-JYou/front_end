...
class WxPayNotify extends \app\common\lib\pay\wxpay\WxPayNotifyReply {
  ...
  public function NotifyProcess($data, &$msg) {
    ...
    if (substr($data['out_trade_no'], 0, 2) == FreelanceOrder::PREFIX_ORDER) {
      $ptOrder = new FreelanceOrder();
      $order = $ptOrder->where('order_sn', $data['out_trade_no'])->find();
      if ($order) {
        return $ptOrder->paid($order, 'wxpay', $data['total_fee'], $data['transaction_id']);
      }
    }
    if (substr($data['out_trade_no'], 0, 2) == CityinfoOrder::PREFIX_ORDER) {
      $ptOrder = new CityinfoOrder();
      $order = $ptOrder->where('order_sn', $data['out_trade_no'])->find();
      if ($order) {
        return $ptOrder->paid($order, 'wxpay', $data['total_fee'], $data['transaction_id']);
      }
