...
class WxPayApi {
  ...
  public static function unifiedOrder($inputObj, $timeOut = 6) {
    ...
    if (
      $inputObj->GetTrade_type() == 'NATIVE' &&
      !$inputObj->IsProduct_idSet()
    ) {
      throw new WxPayException(
  ...
  public static function micropay($inputObj, $timeOut = 10) {
    ...
    if (!$inputObj->IsBodySet()) {
      throw new WxPayException(
        '提交被扫支付API接口中，缺少必填参数body！'
      );
    } elseif (!$inputObj->IsOut_trade_noSet()) {
      throw new WxPayException(
        '提交被扫支付API接口中，缺少必填参数out_trade_no！'
      );
    } elseif (!$inputObj->IsTotal_feeSet()) {
      throw new WxPayException(
        '提交被扫支付API接口中，缺少必填参数total_fee！'
      );
    } elseif (!$inputObj->IsAuth_codeSet()) {
      throw new WxPayException(
        '提交被扫支付API接口中，缺少必填参数auth_code！'
      );
    }
   ...
   * @throws WxPayException
   */
  private static function postXmlCurl(
    $xml,
    $url,
    $useCert = false,
    $second = 30
    ...
    if ($useCert == true) {
      ...
      curl_setopt(
        $ch,
        CURLOPT_SSLCERT,
        getcwd() . '/data/cert/apiclient_cert.pem'
      );
      curl_setopt($ch, CURLOPT_SSLKEYTYPE, 'PEM');
      curl_setopt(
        $ch,
        CURLOPT_SSLKEY,
        getcwd() . '/data/cert/apiclient_key.pem'
      );
    }
    //post提交方式
