...
class FileManager {
  ...
  /** 上传文件 */
  public function uploadReturnPath($file, $full = null) {
    if (!$file) {
      $this->_error = '请上传文件';
      return false;
    }
    ...
    if ($info) {
      $return = [
        'save_path' => make_file_url(
          $this->uploadfile_dir . '/' . $info->getSaveName(),
          null,
          $full
        )
      ];
  ...
  /** 上传视频文件 */
  public function uploadVideoReturnPath($file, $full = null) {
    if (!$file) {
      $this->_error = '请上传文件';
      return false;
    }
  ...
  /** 上传文件 */
  public function upload(File $file, int $uid, int $utype, $full = true) {
    if (!$file) {
      $this->_error = '请上传文件';
      return false;
    }
    ...
    if (false !== $upload_result) {
      $file_id = $this->recordToDb($upload_result['save_path'], $uid, $utype);
      return [
  ...
  public function save_path($file_path, int $uid, int $utype, string $fileupload_type) {
    $file_id = $this->recordToDb($file_path, $uid, $utype, $fileupload_type);
    return [
      'file_url' => make_file_url(
        $file_path,
        $fileupload_type
  ...
  /** 上传视频 */
  public function uploadVideo($file, int $uid, int $utype) {
    if (!$file) {
      $this->_error = '请选择文件';
      return false;
    }
    ...
    if ($file_url) {
      $file_id = $this->recordToDb($file_url, $uid, $utype, 'qiniu');
      return [
        'file_url' => make_file_url(
          $file_url,
          'qiniu'
  ...
  protected function _upload_default($file) {
    if ($file && stripos($file->getInfo('type'), 'video') !== false) {
      return $this->uploadVideoReturnPath($file);
    }
    $info = $file->validate($this->validate)->move($this->uploadfile_path);
    if ($info) {
      try {
        if ($this->filter == 1) {
          $image = \think\Image::open($this->uploadfile_path . '/' . $info->getSaveName());
          // 返回图片的宽度
          $width = $image->width();
          // 返回图片的高度
          $height = $image->height();
          $image->thumb($width, $height)->save($this->uploadfile_path . '/' . $info->getSaveName());
        }
      } catch (\Throwable $th) {
      }
      return ['save_path' => $this->uploadfile_dir . '/' . $info->getSaveName()];
    } else {
      // 上传失败获取错误信息
      $this->_error = $file->getError();
      return false;
  ...
  /** 记录到数据表 */
  public function recordToDb($save_path, int $uid, int $utype, string $fileupload_type = '') {
