...
class Poster
{
    ...
    /** 今日招聘海报 */
    public function recruitmentToday($index,$id){

        $info = model('RecruitmentToday')
            ->where('id', 'eq', $id)
            ->find();
        if($info===null){
            $this->error = '没有找到今日招聘信息';
            return false;
        }
        $filename = $id.'_'.$info['refreshtime'].'_'.$index.'.jpg';
        $save_dir = SYS_UPLOAD_PATH.'poster/recruitmentToday/'.($id%10).'/';
        if(!is_dir($save_dir)){
            mkdir($save_dir,0777,true);
        }
        $save_path = $save_dir.$filename;
        $show_path = config('global_config.sitedomain').config('global_config.sitedir').SYS_UPLOAD_DIR_NAME.'/poster/recruitmentToday/'.($id%10).'/'.$filename;
        $locationUrl = config('global_config.mobile_domain').'dailyDetail?id='.$info['id'];
        $info['qrcode'] = config('global_config.sitedomain').config('global_config.sitedir').'v1_0/home/qrcode/index?alias=subscribe_dailyDetail&url='.$locationUrl.'&recruitment_today_id='.$info['id'];
        $config = [
            'image'=>[
                [
                    'url'=>$info['qrcode'],//二维码
            ...
            'text'=>[
                [
                    'text'=>config('global_config.sitename'),
                    'fontPath'=>API_LIB_PATH.'poster/recruitment.otf',
                    'top'=>-1153,
                    'center_x'=>1,
                    'fontSize'=>55,       //字号
                    'fontColor'=>'255,255,255',//字体颜色
                ...
                [
                    'text'=>'来自'.config('global_config.sitename'),
                    'fontPath'=>$this->fontPath,
                    'center_x'=>1,
                    'top'=>-78,
                    'fontSize'=>22,       //字号
            ...
            'background'=>SYS_UPLOAD_PATH.'resource/poster/recruitment/'.$index.'.png' //背景图
        ];
        $result = $this->create($config,$save_path);
        if($result===false){
            $this->error = '生成海报失败';
    ...
    public function recruitmentTodayLogo($id,$type = 'pc'){
        $info = model('RecruitmentToday')
            ->where('id', 'eq', $id)
            ->find();
        if($info===null){
        ...
        {
            $logo = 'todayBg.png';
            $filename = $id.'_logo.jpg';
        }elseif($type == 'list')
        {
            $logo = 'dailyHeadImg.png';
            $filename = $id.'_list_logo.jpg';
        }
            else
        {
            $logo = 'dailyHeadImg.jpg';
            $filename = $id.'_mobile_logo.jpg';
        }
        $save_dir = SYS_UPLOAD_PATH.'recruitmentToday/'.($id%10).'/'.$id.'/';
        if(!is_dir($save_dir)){
            mkdir($save_dir,0777,true);
        }
        $save_path = $save_dir.$filename;
        $show_path = config('global_config.sitedomain').config('global_config.sitedir').SYS_UPLOAD_DIR_NAME.'/recruitmentToday/'.($id%10).'/'.$id.'/'.$filename;
        $config = [
            'image'=>[
                [
                    'url'=>$_SERVER['DOCUMENT_ROOT']. '/assets/images/recruitment/'.$logo,//文字背景
                    'left'=>55,
                    'top'=>1350,
                    'width'=>970,
                    'height'=>460,
            ...
            'background'=>$_SERVER['DOCUMENT_ROOT']. '/assets/images/recruitment/'.$logo //背景图
        ];
        if ($type == 'pc')
        {
            $config['text'] = [
     ...
     * @return
     */
    public function redEnvelopes($id){
        if($id == null){
            return false;
        }
        $filename = 'envelopes'.time().'.jpg';
        $save_dir = SYS_UPLOAD_PATH.'poster/envelopes/';
        if(!is_dir($save_dir)){
            mkdir($save_dir,0777,true);
        }
        $save_path = $save_dir.$filename;
        ...
        if($member['utype'] == 2){
           ...
           $username = $resume['fullname'];
        }
        $member_balance = model('MemberBalance')->where('uid',$id)->find();
        $show_path = config('global_config.sitedomain').config('global_config.sitedir').'upload/poster/envelopes/'.$filename;
        $config = [
            'image'=>[
                [
                    'url'=>config('global_config.sitedomain').config('global_config.sitedir').'v1_0/home/qrcode/index?type=normal&url='.config('global_config.sitedomain').config('global_config.sitedir').'m/member/reg?invitation_code='.$member_balance['token'],//H5二维码
                    'left'=>240,
                    'top'=>930,
                    'width'=>270,
                    'height'=>270,
            ...
            'text'=>[
                [
                    'text'=>$username.'邀请您注册'.config('global_config.sitename'),
                    'fontPath'=>$this->fontPath,
                    'left'=>225,
                    'center_x'=>1,
                    'top'=>86,
            ...
            'background'=> config('global_config.sitedomain').config('global_config.sitedir').'assets/images/bj.png'//背景图
        ];
        $result = $this->create($config,$save_path);
        if($result===false){
            $this->error = '生成海报失败';
