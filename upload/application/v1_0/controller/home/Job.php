...
class Job extends \app\v1_0\controller\common\Base {
  ...
  public function show($a = null, $b = null, $c = null, $d = null) {
    ...
    if (intval(config('global_config.alicloud_phone_protect_open'))) {
      ...
      if ($return['phone_protect_type'] == 1 && !$this->userinfo->uid) {
   ...
   * @author chenyang
   * @return array
   * Date Time：2022年5月6日17:33:21
   */
  public function saveRegisterData() {
    // 校验请求参数
    $verifyResult = $this->_verifyParams();
    if ($verifyResult['status'] === false) {
      $this->ajaxReturn(500,  $verifyResult['msg']);
    }

    $jobRegisterModel = Model('JobRegister');
    // 开启事务
    $jobRegisterModel->startTrans();
    try {
      $insertData = [
        'handle_status' => 0,
        'add_time'      => time(),
      ];
      $registerId = $jobRegisterModel->add($insertData);
      if (empty($registerId)) {
        throw new \Exception('保存求职登记信息失败-请求SQL为：' . $jobRegisterModel->getLastSql());
      }
      $insertContentData = [];
      foreach ($verifyResult['data'] as $value) {
        $insertContentData[] = [
          'register_id' => $registerId,
          'field_id'    => $value['field_id'],
          'field_alias' => $value['field_alias'],
          'content'     => $value['content'],
        ];
      }
      $contentModel = Model('JobRegisterContent');
      $result = $contentModel->addAll($insertContentData);
      if (empty($result)) {
        throw new \Exception('保存求职登记内容信息失败-请求SQL为：' . $contentModel->getLastSql());
      }

      // 提交事务
      $jobRegisterModel->commit();
    } catch (\Exception $e) {
      // 回滚事务
      $jobRegisterModel->rollback();
      saveLog('保存失败-报错信息：' . json_encode(['Line' => $e->getLine(), 'File' => $e->getFile(), 'Message' => $e->getMessage()]));
      $this->ajaxReturn(500,  '保存失败');
    }
  ...
  private function _verifyParams() {
    ...
    $fieldList = $templateFieldModel->getJoinFieldList($where, $field);
    if (empty($fieldList)) {
      return callBack(false, '模板字段不存在');
    }

    $data = [];
    foreach ($fieldList as $fieldInfo) {
      // 校验必填项
      if ($fieldInfo['is_must'] == 1) {
        if (!isset($params[$fieldInfo['field_alias']]) || empty($params[$fieldInfo['field_alias']])) {
          return callBack(false, '请填写' . $fieldInfo['field_name']);
        }
      }
      // 校验验证码
      if ($fieldInfo['field_way'] == 2) {
        if (!isset($params['verify_code']) || empty($params['verify_code'])) {
          return callBack(false, '请填写验证码');
        }
        // 校验验证码
        $smsCodeKey = 'smscode_' . $params['contact_way'];
        $smsCode = cache($smsCodeKey);
        if ($smsCode === false || $smsCode['code'] != $params['verify_code']) {
          return callBack(false, '验证码错误');
        }
      }
      // 拼装新增时的数据
      if (isset($params[$fieldInfo['field_alias']]) && !empty($params[$fieldInfo['field_alias']])) {
        ...
        if (in_array($fieldInfo['field_type'], [1, 5]) && iconv_strlen($params[$fieldInfo['field_alias']]) > $fieldLength) {
          return callBack(false, $fieldInfo['field_name'] . '长度不能超过' . $fieldLength . '个字符');
        }
        $content = $params[$fieldInfo['field_alias']];
        if (is_array($content)) {
          $fieldContent = ',';
    ...
    if (empty($data)) {
      return callBack(false, '未获取到保存时的数据');
    }
  ...
  public function getRegisterViewData() {
    ...
    if (empty($templateInfo)) {
      ...
      if ($result['status'] === false) {
        $this->ajaxReturn(500, '新增默认模板失败，请联系网站管理员');
      }
    ...
    if (empty($fieldList)) {
      $this->ajaxReturn(500, '模板字段不存在，请联系网站管理员');
    }
    // 获取字段内容
    ...
    $return = [
      ...
      'base_data'  => [
        ...
        'contact_tel' => $configData['contact_tel'],
      ],
    ];
    $this->ajaxReturn(200, '获取成功', $return);
  }
}
