...
class Config extends \app\v1_0\controller\common\Base {
  ...
  public function index() {
    ...
    $img_id_arr = [$list['logo'], $list['square_logo'], $list['wechat_qrcode'], $list['wechat_miniprogram_qrcode'], $list['cityinfo_kefu']];
    ...
    $list['cityinfo_kefu'] =  isset($img_arr[$list['cityinfo_kefu']]) ? $img_arr[$list['cityinfo_kefu']] : make_file_url('resource/weixin_img.jpg');
    $list['wechat_miniprogram_qrcode'] = isset($img_arr[$list['wechat_miniprogram_qrcode']]) ? $img_arr[$list['wechat_miniprogram_qrcode']] : '';
    $list['link_url_web'] = [
      ...
      'jobfairlist' => url('index/jobfair/index'),
      ...
      'freelance' => url('index/freelance/index'),
      'freelance_resume' => url('index/freelance/resume'),
      'freelance_subject' => url('index/freelance/subject'),
      'jobfairollist' => url('index/jobfairol/index'),
      'campus' => url('index/campus/index'),
      'fast_job' => url('index/fast/job'),
      'fast_resume' => url('index/fast/resume')
    ];
    $list['subsite_list'] = [];
    ...
    $d = model('Config')->where('name', 'like', 'ico_%')->column('name,value');
    foreach ($d as $k => $v) {
      $list[$k] = $v;
    }
    $config_payment = config('global_config.account_alipay');
    $list['live_app_key'] = config('global_config.live_app_key');
    $list['live_app_secret'] = config('global_config.live_app_secret');
  ...
  public function userinfo() {
    $return['login'] = $this->userinfo->uid;
  ...
  public function version() {
    $type = input('get.type/s', 'android', 'trim');
    if ($type == 'android') {
      $return['version'] = config('global_config.app_android_version');
      $return['content'] = config('global_config.app_android_upgrade_content');
      $return['downurl'] = config('global_config.app_android_download_url');
      $return['force'] = config('global_config.app_android_upgrade_force');
      $return['force'] = $return['force'] == 1 ? true : false;
    } else {
      $return['version'] = config('global_config.app_ios_version');
      $return['content'] = config('global_config.app_ios_upgrade_content');
      $return['downurl'] = config('global_config.app_ios_download_url');
      $return['force'] = config('global_config.app_ios_upgrade_force');
      $return['force'] = $return['force'] == 1 ? true : false;
    }
    $this->ajaxReturn(200, '获取数据成功', $return);
  }
