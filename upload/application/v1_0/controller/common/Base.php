...
class Base extends \app\common\controller\Base {
  ...
  private function randomField($source, $set = false, $fields = ['u0', 'uj', 'm0', 'mz']) {
    $isarr = is_array($source);
    if ($set) {
      if ($isarr) {
        foreach ($fields as $v) {
          $source[$v] = mt_rand() % 2 ? mt_rand() : '';
        }
      } else {
        foreach ($fields as $v) {
          $source->$v = mt_rand() % 2 ? mt_rand() : '';
    ...
    if ($user_token) {
      try {
        ...
        if ($auth_result['code'] == 200) {
          $this->userinfo = new Userinfo($this->randomField($auth_result['info']));
        }
      ...
      if ($this->userinfo !== null) {
        $refresh_result = model('IdentityToken')->refreshToken($user_token, $this->userinfo);
        if ($refresh_result === false && !Config::get('app_debug')) {
          $this->userinfo = new Userinfo(null);
        }
      }
    }
  ...
  public function loginExtra($uid, $utype, $mobile) {
    ...
    $u = model('Member')->get($uid);
    $rid = $u ? $u->rid : 0;
    $ret = ['uid' => $uid, 'rid' => $rid, 'utype' => $utype, 'mobile' => $mobile,];
    // 设置干扰
    $ret = $this->randomField($ret, true);
    ...
    $visitor->setLogin($ret, $this->expire_platform[config('platform')]);
    $ret['next_code'] = $next_code;
    return $ret;
  }
  /**  拦截企业基本资料 */
