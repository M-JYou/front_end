...
class Resume extends \app\v1_0\controller\common\Base {
  ...
  public function basicSave() {
    ...
    $resume_arr = model('Resume')->where('uid', $this->userinfo->uid)->find();
    if ($resume_arr['audit'] == 0) {
      $sex = model('Resume')->map_sex[$resume_arr['sex']];
      $age = date('Y') - intval($resume_arr['birthday']);
      $education = model('Resume')->map_education[$resume_arr['education']];
      $birthday = $resume_arr['enter_job_time'] == 0
        ? '无经验'
        : format_date($resume_arr['enter_job_time']);;
      model('AdminNotice')->send(
        3,
        '简历审核通知',
        '简历ID:【' . $resume_arr['id'] . '】' . "\r\n" .
          '姓名:【' . $resume_arr['fullname'] . '】' . "\r\n" .
          '基本信息:【' . $sex . '/' . $age . '岁/' . $education . '/' . $birthday . '】' . "\r\n" .
          '简历完整度:【' . model('Resume')->countCompletePercent($resume_arr['id']) . '%】',
        '简历审核通知，请及时跟进（后台->用户->个人简历管理->未审核简历）'
      );
    }
    ...
    model('MemberBalance')->moneyRecord(
      ...
      '简历完善',
      model('Resume')->countCompletePercent(0, $this->userinfo->uid)
    );
  ...
  public function uploadPhoto() {
    ...
    $result = $filemanager->upload($file, $this->userinfo->uid, 0, input('post.full') !== null);
    if (false !== $result) {
      $resume = model('Resume')
        ->where('uid', $this->userinfo->uid)
        ->find();
  ...
  public function imgAdd() {
    ...
    if (false !== $result) {
      ...
      model('AdminNotice')->send(
        9,
        '照片作品审核通知',
        '所属个人:【' . $resume['fullname'] . '】' . "\r\n" .
          '联系电话:【' . $this->userinfo->mobile . '】',
        '照片作品审核通知，请及时跟进（后台->用户->个人简历管理->照片/作品）'
      );
