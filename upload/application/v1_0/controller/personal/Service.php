...
class Service extends \app\v1_0\controller\common\Base {
  ...
  public function pay() {
    ...
    $result = model('Order')->''=>'integer', //  ($input_data);
    if (false === $result) {
    ...
    model('AdminNotice')->send(
      ...
      '订单号:【' . $result['order_oid'] . '】' . "\r\n" .
        '个人名称:【' . $member['username'] . '】' . "\r\n" .
        '服务内容:【' . model('Order')->map_service_type_personal[$input_data['service_type']] . '】' . "\r\n" .
        '订单金额:【' . $order['amount'] . '】' . "\r\n" .
        '订单状态:【' . model('Order')->map_status[$order['status']] . '】',
      '个人订单通知，请及时跟进（后台->业务->个人业务管理->订单管理）'
    );
  ...
  public function orderDetail() {
    ...
    $info['service_type_text'] = model('Order')->map_service_type_personal[$info['service_type']];
