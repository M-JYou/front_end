...
class Im extends \app\v1_0\controller\common\Base {
  protected $n = '[IM]';
  protected $baseUrl = 'https://imserv.v2.74cms.com';
  protected $config = [
    'app_key' => '',
    'app_secret' => ''
  ];
  ...
  public function imCheckBind() {
    ...
    $bind_data = model('MemberBind')
      ...
      ->find();
    if (null === $bind_data) {
      $this->ajaxReturn(200, $this->n .  '未绑定', 0);
    }
    $this->ajaxReturn(200, $this->n .  '已绑定', 1);
  }
  ...
  private function token() {
    ...
    $result = decode(https_request($url));
    if ($result['code'] != 200) {
      $result['result'] = '';
    }
    return $result;
  ...
  public function getToken() {
    $result = $this->token();
    if ($result['code'] == 500) {
      $this->ajaxReturn(500, $this->n .  $result['msg']);
    }
    $this->ajaxReturn(200, $this->n .  '获取数据成功:' . $this->userinfo->utype . ' ' . $this->userinfo->uid, $result['result']);
  }
  ...
  public function imWindowGlobal() {
    ...
    if ($checkWindowGlobal['code'] == 0) {
      $this->ajaxReturn(200, $this->n .  $checkWindowGlobal['message'], ['next' => $checkWindowGlobal['next']]);
    }
    $this->ajaxReturn(200, $this->n .  '检测通过', ['next' => '']);
  }
  // 查询今日已发起聊天次数
  ...
  protected function checkAuth() {
    ...
    if (!$appkey) {
      $this->ajaxReturn(500, $this->n .  '[IM]appkey不能为空');
    }
    if (!$appsecret) {
      $this->ajaxReturn(500, $this->n .  'appsecret不能为空');
    }
    if ($appkey != $this->config['app_key'] || $appsecret != $this->config['app_secret']) {
      $this->ajaxReturn(500, $this->n .  'appkey或appsecret错误');
    }
  }
  // 发起聊天
  public function startConversation() {
    ...
    $token = input('post.token/s', $this->token()['result'], 'trim');
    if (!$token) {
      $this->ajaxReturn(500, $this->n .  'token不能为空');
    }
    $target = model('MemberInfo')->where('id', input('param.target/s', '', 'trim'))->find();
    ...
    return https_request($this->baseUrl . '/chat/startConversation', $data);
  ...
  public function chatList() {
    ...
    $this->ajaxReturn(200, $this->n .  '获取数据成功', ['items' => $return]);
  }
  // 聊天记录
  ...
  public function jobDetail() {
    ...
    $return['experience_text'] = isset(
      ...
      ? model('BaseModel')->map_experience[$jobinfo['experience']]
      : '经验不限';
    $this->ajaxReturn(200, $this->n .  '获取数据成功', $return);
  }
  // 职位联系方式
  public function jobContact() {
    ...
    $this->ajaxReturn(200, $this->n .  '获取数据成功', $contact_info);
  }
  // 企业联系方式
  ...
  public function removeChat() {
    ...
    $this->ajaxReturn(200, $this->n .  '删除成功');
  }
  // 获取当前用户信息
  ...
  public function jobinfo() {
    ...
    $jobinfo = model('JobSearchRtime')
      ...
      ->find();
    if ($jobinfo === null) {
      $this->ajaxReturn(200, $this->n .  '请选择职位', ['next' => 'choose_job']);
    }
  ...
  public function phraseList() {
    ...
    $this->ajaxReturn(200, $this->n .  '获取数据成功', ['items' => $result['result']]);
  }
  // 修改即时通讯快捷用语
  public function phraseEdit() {
    ...
    $this->ajaxReturn(200, $this->n .  '保存成功');
  }
  // 添加即时通讯快捷用语
  public function phraseAdd() {
    ...
    $this->ajaxReturn(200, $this->n .  '保存成功', $result['result']);
  }
  // 删除即时通讯快捷用语
  ...
  public function joblist() {
    ...
    $this->ajaxReturn(200, $this->n .  '获取数据成功', ['items' => $list]);
  }
  // 切换职位
  public function changejob() {
    ...
    $this->ajaxReturn(200, $this->n .  '操作成功');
  }
  // 置顶聊天
  ...
  public function blacklistDel() {
    ...
    $this->ajaxReturn(200, $this->n .  '移出成功');
  }
  // 黑名单添加
  public function blacklistAdd() {
    $this->checkLogin();
    $jobname = input('post.jobname/s', '1', 'trim');
    $chatid = input('post.chatid/s', '', 'trim');
    $token = input('post.token/s', '', 'trim');
    $url = $this->baseUrl . '/blacklist/add';
    $result = https_request($url, ['token' => $token, 'chatid' => $chatid/* , 'jobid' => $jobname */]);
    return $result;
  }
  // 检查是否在黑名单
  public function blacklistCheck() {
    ...
    $this->ajaxReturn(200, $this->n .  $result['msg'], $result['result']);
  }
  // 获取面试信息
  public function interviewInfo() {
    ...
    if ($this->userinfo->utype == 1) {
      if (!$resumeid) {
        $this->ajaxReturn(500, $this->n .  '请选择');
      }
      ...
      if (null === $jobinfo) {
        $this->ajaxReturn(500, $this->n .  '没有找到职位信息');
      }
