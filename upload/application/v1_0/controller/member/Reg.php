...
class Reg extends \app\v1_0\controller\common\Base {
  ...
  public function company() {
    $input_data = [
      ...
      'invitation_code' => input('post.invitation_code/s', '', 'trim')
    ];
    ...
    $invite_register_is_open = config('global_config.invite_register')['is_open'];
    if ($input_data['invitation_code'] != '' && $invite_register_is_open == 1) {
      $member_balance = model('MemberBalance')->where('token', $input_data['invitation_code'])->find();
      model('MemberInviteRegister')->save([
        'pid' => $member_balance['uid'],
    ...
    model('AdminNotice')->send(
      1,
      '企业注册通知',
      '企业UID【' . $reg_userinfo['uid'] . '】' . "\r\n" .
        '企业名称:【' . $input_data['companyname'] . '】' . "\r\n" .
        '联系电话【' . $input_data['mobile'] . '】' . "\r\n" .
        '注册来源【' . model('AdminNotice')->map_platform[config('platform')] . '】',
      '企业用户注册，请及时跟进（后台->用户->会员管理->企业会员）'
    );
  ...
  public function personal() {
    ...
    model('AdminNotice')->send(
      2,
      '个人注册通知',
      '个人UID【' . $reg_userinfo['uid'] . '】' . "\r\n" .
        '个人名称:【user_' . $input_data['mobile'] . '】' . "\r\n" .
        '联系电话【' . $input_data['mobile'] . '】' . "\r\n" .
        '注册来源【' . model('AdminNotice')->map_platform[config('platform')] . '】',
      '个人用户注册，请及时跟进（后台->用户->会员管理->个人会员）'
    );
    $invite_register_is_open = config('global_config.invite_register')['is_open'];
    if ($input_data['invitation_code'] != '' && $invite_register_is_open == 1) {
      $member_balance = model('MemberBalance')->where('token', $input_data['invitation_code'])->find();
  ...
  public function personalQuick() {
    ...
    if (false === model('JobApply')->jobApplyAdd($job_apply_data, $reg_userinfo['uid'])) {
