...
class Upload extends \app\v1_0\controller\common\Base {
  ...
  public function index() {
    ...
    $result = $filemanager->upload($file, $this->userinfo->uid, 0, !input('?post.short'));
    if (false !== $result) {
      if ($extra == 'company_logo') {
        cache('scan_upload_result_company_logo_' . $this->userinfo->uid, json_encode($result));
      }
      if (input('?get.dir')) {
        die(encode(['error' => 0, 'url' => $result['file_url']]));
      }
      $this->ajaxReturn(200, '上传成功', $result);
    } else {
      if (input('?get.dir')) {
        die(encode(['error' => 1, 'message' => $filemanager->getError()]));
      }
  ...
  protected function video() {
    ...
    config('global_config.fileupload_size', 1000 * 1024 * 1024);
    if (!$file) {
      $this->ajaxReturn(500, '请选择文件');
    }
    $filemanager = new \app\common\lib\FileManager();
    $result = $filemanager->upload($file, $this->userinfo->uid, $this->userinfo->utype, !input('?post.short'));
    if (false !== $result) {
      $this->ajaxReturn(200, '上传成功', $result);
    } else {
      $this->ajaxReturn(500, $filemanager->getError());
  ...
  public function save_qiniu_file() {
    ...
    try {
      ...
      $filemanager = new \app\common\lib\FileManager();
      $result = $filemanager->save_path($file_path, $this->userinfo->uid, 0, 'qiniu');
      $this->ajaxReturn(200, '上传成功', $result);
    } catch (Exception $e) {
