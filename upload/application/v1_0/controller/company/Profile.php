...
class Profile extends \app\v1_0\controller\common\Base {
    ...
    if (request()->isGet()) {
      ...
      try {
        if ($company_profile === null) {
          ...
          $result = model('Company')->u($input_data['basic'], ['id' => $company_profile['id'],]);
        ...
        if ($company_info != null && $company_profile['audit'] == 1 && config('global_config.audit_edit_com') == 1) {
  ...
  public function uploadImg() {
    ...
    $result = $filemanager->upload($file, $this->userinfo->uid, 0);
    if (false !== $result) {
      ...
      model('AdminNotice')->send(
        9,
        '企业风采审核通知',
        '所属企业:【' . $this->company_profile['companyname'] . '】' . "\r\n" .
          '联系电话:【' . $this->userinfo->mobile . '】',
