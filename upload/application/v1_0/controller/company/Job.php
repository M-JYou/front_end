...
class Job extends \app\v1_0\controller\common\Base {
  ...
  public function addSave() {
    ...
    if (input('?post.basic.department')) {
      $input_data['basic']['department'] = input('post.basic.department/s', '', 'trim,badword_filter');
    } else {
    ...
    if (
      ...
      $input_data['basic']['minage'] = input('post.basic.minage/d', 0, 'intval');
      $input_data['basic']['maxage'] = input('post.basic.maxage/d', 0, 'intval');
      $input_data['basic']['age_na'] = input('post.basic.age_na/d', 0, 'intval');
    } else {
    ...
    if (input('?post.basic.amount')) {
      $input_data['basic']['amount'] = input('post.basic.amount/d', 0, 'intval');
    ...
    if (input('?post.basic.custom_field_1')) {
      $input_data['basic']['custom_field_1'] = input('post.basic.custom_field_1/s', '', 'trim,badword_filter');
    } else {
      $input_data['basic']['custom_field_1'] = '';
    }
    if (input('?post.basic.custom_field_2')) {
      $input_data['basic']['custom_field_2'] = input('post.basic.custom_field_2/s', '', 'trim,badword_filter');
    } else {
      $input_data['basic']['custom_field_2'] = '';
    }
    if (input('?post.basic.custom_field_3')) {
      $input_data['basic']['custom_field_3'] = input('post.basic.custom_field_3/s', '', 'trim,badword_filter');
    } else {
    ...
    //新添加的职位，根据配置赋值审核状态
    $input_data['basic']['audit'] = $this->company_profile['audit'] == 1
    ...
    if (input('?post.contact.weixin')) {
      $input_data['contact']['weixin'] = input('post.contact.weixin/s', '', 'trim,badword_filter');
    } else {
      $input_data['contact']['weixin'] = '';
    }
    if (input('?post.contact.telephone')) {
      $input_data['contact']['telephone'] = input('post.contact.telephone/s', '', 'trim,badword_filter');
    } else {
      $input_data['contact']['telephone'] = '';
    }
    if (input('?post.contact.qq')) {
      $input_data['contact']['qq'] = input('post.contact.qq/s', '', 'trim,badword_filter');
    } else {
      $input_data['contact']['qq'] = '';
    }
    if (input('?post.contact.email')) {
      $input_data['contact']['email'] = input('post.contact.email/s', '', 'trim,badword_filter');
    } else {
    ...
    if ($input_data['basic']['audit'] == 1) {
      $service_stick_list = model('CompanyServiceStick')->getList(3, $this->userinfo->uid);
    } else {
      $service_stick_list = [];
    }
    if (isset($input_data['basic']['audit']) && $input_data['basic']['audit'] == 0) {
      $wage_text = model('BaseModel')->handle_wage(
        $input_data['basic']['minwage'],
        $input_data['basic']['maxwage'],
        $input_data['basic']['negotiable']
      );
      $education = isset(model('BaseModel')->map_education[$input_data['basic']['education']])
        ? model('BaseModel')->map_education[$input_data['basic']['education']]
        : '';
      $experience = isset(model('BaseModel')->map_experience[$input_data['basic']['experience']])
        ? model('BaseModel')->map_experience[$input_data['basic']['experience']]
        : '';
      model('AdminNotice')->send(
        5,
        '职位审核通知',
        '职位ID:【' . $jobid . '】' . "\r\n" .
          '职位名称:【' . $input_data['basic']['jobname'] . '】' . "\r\n" .
          '职位薪资:【' . $wage_text . '】' . "\r\n" .
          '职位要求:【' . $education . '/' . $experience . '】' . "\r\n" .
          '公司名称:【' . $this->company_profile['companyname'] . '】',
        '您有一条职位待审核，请及时跟进（后台->用户->企业信息管理->待审核职位）'
      );
    }
  ...
  public function index() {
    $type = input('param.type/d', 0, 'intval');
    $current_page = input('param.page/d', 1, 'intval');
    $pagesize = input('param.pagesize/d', 5, 'intval');
    $list = model('Job')
      ->field(
  ...
  public function getCategoryJobTemplate() {
    $pid = input('param.pid/d', 0, 'intval');
