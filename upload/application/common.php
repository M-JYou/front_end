...
/**  生成上传文件预览地址 */
function make_file_url($path, $type = 'default', $full = true) {
  if ($path == '') {
    return '';
  }
  ...
  switch ($type) {
    case 'default':
      return ($full ? config('global_config.sitedomain') : '') . config('global_config.sitedir') . SYS_UPLOAD_DIR_NAME . '/' . $path;
      break;
    case 'qiniu':
    ...
    default:
      $t = stripos($path, 'http');
      if ($t === false || $t > 0) {
        $t = stripos($path, 'upload');
        if ($t === false || $t > 0) {
          $path = 'upload/' . $path;
        }
        return config('global_config.sitedomain') . '/' . $path;
      }
      return $path;
      break;
  }
...
function default_empty($alias) {
  switch ($alias) {
    ...
    case 'jobfair_banner':
      ...
      break;
    case 'job_search_group': // 求职群
      $filename = 'job_search_group.png';
      break;
    default:
      $filename = '';
      break;
...
function U($id) {
  $request = \think\Request::instance();
  $t = strtolower($request->module()) . '/' . strtolower($request->controller()) . '/' . strtolower($request->action());
  $k = strval(intval(microtime() * 1000000));
  $t = url($t, [$k => $id]);

  return str_replace("/$k/", '/', $t);
}
/**  [ddate 时间差] */
function ddate($s, $e) {
...
function curl_file_get_contents($durl, $appkey = '', $appsecret = '') {
  // 初始化
  $headers = [
    'appkey:' . $appkey,
    'appsecret:' . $appsecret
 ...
 */
function curl_file_post_contents($durl, $post_data, $appkey = '', $appsecret = '') {
  //初始化
  $headers = [
    'appkey:' . $appkey,
    'appsecret:' . $appsecret
...
define('IsDebug', \think\Config::get('app_debug'));
function isDebug() {
  return IsDebug;
}
function checkArray(array $safeKeys = [], $source) {
  if (!is_array($source)) {
    $source = input('param.');
  }
  $ret = [];
  foreach ($source as $key => $value) {
    if ($value !== null && in_array($key, $safeKeys)) {
      $ret[$key] = $value;
    }
  }
  return $ret;
...
function ext($code = null, string $message = '', $data = null) {
  $ret = func_num_args() < 2 ?
    (isset($code['code']) && isset($code['message']) ?
      $code : ['code' => 200, 'message' => '获取数据成功', 'data' => $code]
    ) : ['code' => $code === 0 ? 500 : $code, 'message' => $message, 'data' => $data];
...
function getCityByIp($ip, int $len = 0) {
  $return = '630100';
  do {
    if (!$ip || $ip == '127.0.0.1') {
      break;
    ...
    $return = isset($r->content->address_detail->adcode) ? substr($r->content->address_detail->adcode, 0, 6) : '630100';
    if (isset($r->content->address_detail->adcode)) {
      $return = substr($r->content->address_detail->adcode, 0, 6);
    } elseif (isset($r->content->address_detail->province) && isset($r->content->address_detail->city)) {
      $return = mb_substr($r->content->address_detail->province, 0, mb_strlen($r->content->address_detail->province) - 1);
      $m = model('CategoryDistrict');
      $d = $m->where('name', 'like', mb_substr($r->content->address_detail->province, 0, mb_strlen($r->content->address_detail->province) - 1) . '%')
        ->order('sort_id desc,id asc')->find();
      if (isset($d->id)) {
        $d = $m->where('pid', $d->id)
          ->where('name', 'like', mb_substr($r->content->address_detail->city, 0, mb_strlen($r->content->address_detail->city) - 1) . '%')
          ->order('sort_id desc,id asc')->find();
        if (isset($d->id)) {
          $return = $d->id;
        }
    ...
    cache('ip_city_' . $ip, $return, 2592000);
    break;
  } while (false);
  return $len > 0 ? substr($return, 0, $len) : $return;
}
function getCityId() {
  $r = cookie('cityId');
  if (!$r) {
    $r = getCityByIp(get_client_ip());
    cookie('cityId', $r);
  }
  return $r;
}
function boolean($p) {
  $r = strtolower(strval($p));
  return !in_array($r, ['false', '0', 'null', 'undefined']);
}
function trimAll(&$arr) {
  if (is_array($arr)) {
    foreach ($arr as $key => $value) {
      $arr[$key] = trimAll($value);
    }
  } else {
    try {
      $arr = decode(trim($arr));
...
function checkFields(array &$obj, array $fields, $strArr = false) {
  foreach ($obj as $k => $v) {
    if (!in_array($k, $fields, true) || $v === null) {
      unset($obj[$k]);
    } elseif ($strArr && is_array($v) && (!is_array($strArr) || (isset($strArr[$k]) && $strArr[$k] == 'string'))) {
      $obj[$k] = encode($v);
    }
  }
  return $obj;
}
function parseJson($param, $def = []) {
  try {
    if (is_string($param)) {
...
function parseArr($param, $def = []) {
  ...
  if (is_array($param)) {
    ...
    $param = trim($param);
    if (strval(floatval($param)) === $param) {
      $ret[] = floatval($param);
    } else {
...
function underline(string $param) {
  return strtolower(trim(preg_replace("/[A-Z]/", "_\\0", $param), "_"));
}
function findArr(array $arr, $value, $key = 'id', $returnKey = false) {
  if (!$key) {
    $key = 'id';
  }
  foreach ($arr as $k => $v) {
  ...
  return $returnKey ? -1 : null;
}
function selectArr(array $arr, $value, $key = 'id', $returnKey = false, $count = 0) {
  $ret = [];
  try {
    if (!($count > 0)) {
      throw null;
...
function parseField(array $arr, $field = ['content']) {
  if (!is_array($field)) {
  ...
  foreach ($ret as $k => $v) {
    foreach ($field as $fk => $fv) {
      ...
      if ($v[$dk] && is_string($v[$dk]) && in_array($v[$dk][0], $pre)) {
        try {
          $ret[$k][$dk] = json_decode($v[$dk], true);
        } catch (\Throwable $th) {
          $ret[$k][$dk] = $def;
        }
      }
...
function getArrField($arr, $key = 'id') {
  $ret = [];
  if (is_array($arr)) {
    foreach ($arr as $v) {
      try {
...
function clo($param) {
  return $param ? json_decode(encode($param), true) : $param;
}
function clearArrNull(array &$param) {
...
function clearArrNullRet(array $param) {
  return clearArrNull($param);
}
...
function encode($v) {
  return is_numeric($v) || is_string($v) ? $v :
    json_encode($v, JSON_UNESCAPED_SLASHES | JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_LINE_TERMINATORS);
}
...
define('TPEXP', [
  ...
  'notbetween time' => 'notbetween time',
]);
function getExp(array $p) {
  return is_array($p) && count($p) == 2 ? TPEXP[$p[0]] . ' ' . $p[1] : '';
}
function isExp($param) {
  $ret = false;
  try {
    if (is_array($param) && count($param) == 2) {
      $ret = is_string($param[0]) && isset(TPEXP[$param[0]]);
    }
  } catch (\Throwable $th) {
  }
  return $ret;
}
function isWhere($param) {
  $ret = false;
  try {
    if (is_array($param)) {
      $ret = !(is_string($param[0]) && in_array($param[0], TPEXP));
    }
  } catch (\Throwable $th) {
  }
  return $ret;
}
function wf($data, string $fileName = null) {
  $fp = './tmp/' . $fileName . '.txt';
  if (is_string($data)) {
    $d = $data;
...
function getLegalOrigin() {
  $r = request()->header('origin');
  $domains = [
    'http://zixia6.com', 'https://zixia6.com',
    'http://www.zixia6.com', 'https://www.zixia6.com',
    'http://test.yzb168.com', 'https://test.yzb168.com',
    'http://ad.cc', 'http://localhost:8080',
    'http://test.zixia6.com',
  ];
  return in_array($r, $domains) ? $r : '';
}
function modelType(string $mn) {
  $ret = $mn . 'Type';
  try {
    model($ret);
  } catch (\Throwable $th) {
...
function ulbar(array $data, string $k) {
  $r = "";
  foreach ($data as $v) {
    $t = is_array($v['children']) ? ' class="children"' : '';
    $r .= '<li' . $t . '><a href="' . P([$k => $v['id']]) . '">' . $v['name'] . '</a><span></span>';
    if ($t) {
      $r .= ulbar($v['children'], $k);
    }
    $r .= '</li>';
  }
  return  "<ul>$r</ul>";
