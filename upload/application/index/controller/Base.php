...
class Base extends \app\common\controller\Base {
  ...
  public function _initialize() {
    parent::_initialize();
    $this->userinfo = (new Visitor())->getLoginInfo();
    $global_config = config('global_config');
    $img_id_arr = [$global_config['logo'], $global_config['square_logo'], $global_config['wechat_qrcode'], $global_config['wechat_miniprogram_qrcode'], $global_config['guide_qrcode']];
    ...
    $global_config['wechat_miniprogram_qrcode'] = isset($img_arr[$global_config['wechat_miniprogram_qrcode']]) ? $img_arr[$global_config['wechat_miniprogram_qrcode']] : '';
    ...
    $this->assign('stxt', '请输入搜索内容');

    // 天气数据
    $dcd = new \app\common\model\CategoryDistrict;
    $this->assign('weather', $dcd->getWeather());

    if ($this->userinfo->uid) {
      $this->pageHeader['messageUnreadSum'] = model('Chat')->where([
  ...
  protected function initPageHeader($global_config) {
    ...
    $this->pageHeader['config'] = $global_config;
    $this->pageHeader['me'] = $this->userinfo;
  }
