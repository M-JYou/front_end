...
class Resume extends \app\index\controller\Base {
  ...
  public function getData() {
    ...
    $mc = model('Category');
    $data = [
      'category_job' => model('CategoryJob')
        ->field('id,name,\'\' style')->select(),
      'citys' => model('CategoryDistrict')->where('pid', cookie('cityId'))
        ->field('*,"" style')->select(),
      'jobtag' => $mc->where('alias', 'QS_jobtag')
        ->field('*,"" style')->select(),
      'trade' => $mc->where('alias', 'QS_trade')
        ->field('*,"" style')->select(),
      'company_type' => $mc->where('alias', 'QS_company_type')
        ->field('*,"" style')->select(),
    ...
    $where = [
      // 'company_id' => count($cid) ?
      //   ['in', model('Company')->where($cid)->column('id')] : null,
      // 'category' => input('param.job/d', null, 'intval'),
      // 'district' => input('param.city/s', null, 'trim'),
    ...
    $data['data'] = [
      'items' => $mj->where($where)
        ...
        ->page($page . ',' . $pagesize)
        ->field('a.*,b.name company_name,b.photo,c.last_login_time')
        // ->fetchSql()
        ->select(),
      'total' => $total,
      'current_page' => $page,
  ...
  protected function writeShowCache($id, $resume_module, $pageCache) {
    ...
    // $t = new \app\common\lib\Tpl($this->visitor);
    // $r = $t->index();
    // $return['flink_list'] = $r['flink_list'];
    return $return;
  }
