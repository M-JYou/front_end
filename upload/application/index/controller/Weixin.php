...
class Weixin extends \app\common\controller\Base {
  ...
  protected function subscribe($object) {
    ...
    if (isset($event['live_id']) && !empty($event['live_id'])) {
      $res =  curl_file_post_contents(
        'https://live.74cms.com/apiadmin/live_public_platform/index',
        [
          'openid' => $data['openid'],
          'subscribe_time' => time(),
  ...
  protected function actionScan($object, $subscribe = 0) {
    ...
    switch ($event['alias']) {
      ...
      case 'sub_fl_resume':
        ...
        $info = $resume->getDetail($id);
        if (!$info) {
          $this->outputText($object, '简历不存在或已删除');
          break;
        }
        $content_arr = [
          "Title" => sprintf('%s,%s,%s,%s', $info['name'], $info['gender_str'], $info['exp_str'], $info['education_str']),
          "Description" => $info['brief_intro'],
          "PicUrl" => model('Uploadfile')->getFileUrl(config('global_config.square_logo')),
          "Url" => config('global_config.mobile_domain') . 'freelance/resume/' . $info['id']
        ];
        $this->outputArticle($object, $content_arr);
        break;
      case 'sub_fl_resume_list':
        $content_arr = [
          "Title" => '【找人才】多元化人才技能服务-' . config('global_config.sitename'),
          "Description" => '自由职业一体化平台',
          "PicUrl" => model('Uploadfile')->getFileUrl(config('global_config.square_logo')),
          "Url" => config('global_config.mobile_domain') . 'freelance/resume'
        ];
        $this->outputArticle($object, $content_arr);
        break;
      case 'sub_fl_subject':
        ...
        $info = $subject->where(['is_public' => 1, 'is_published' => 1, 'audit' => 1, 'id' => $id])->find();
        if (!$info) {
          $this->outputText($object, '项目不存在或已删除');
          break;
        }
        $content_arr = [
          "Title" => sprintf('%s,%s元,%d天', $info['title'], $info['price'] / 100, $info['period']),
          "Description" => $info['desc'],
          "PicUrl" => model('Uploadfile')->getFileUrl(config('global_config.square_logo')),
          "Url" => config('global_config.mobile_domain') . 'freelance/project/' . $info['id']
        ];
        $this->outputArticle($object, $content_arr);
        break;
      case 'sub_fl_subject_list':
        $content_arr = [
          "Title" => '【项目需求】自由接单灵活就业-' . config('global_config.sitename'),
          "Description" => '自由职业一体化平台',
          "PicUrl" => model('Uploadfile')->getFileUrl(config('global_config.square_logo')),
          "Url" => config('global_config.mobile_domain') . 'freelance/project'
        ];
        $this->outputArticle($object, $content_arr);
        break;
      case 'subscribe_job':
      ...
      case 'subscribe_jobfairol':
        ...
        break;
      case 'subscribe_dailyDetail':
        if ($recruitment_today_id = $event['recruitment_today_id']) $recruitment_today = model('RecruitmentToday')->find($recruitment_today_id);
        if (isset($recruitment_today)) {
          if ($sceneQrcodeInfo !== null) {
            $mobile_page = config('global_config.mobile_domain') . model('SceneQrcode')->type_arr[$sceneQrcodeInfo['type']]['mobile_page'];
          } else {
            $mobile_page = config('global_config.mobile_domain') . 'dailyDetail?id=' . $recruitment_today['id'];
          }
          $mobile_page = str_replace(":id", $recruitment_today_id, $mobile_page);
          if (isset($event['scene_uuid'])) {
            $mobile_page .= '&scene_uuid=' . $event['scene_uuid'];
          }
          $wechat_info_img = model('Uploadfile')->getFileUrl(config('global_config.square_logo'));
          $wechat_info_img = $wechat_info_img ? $wechat_info_img : make_file_url('resource/wechat_info_img.jpg');

          $content_arr = [
            "Title" => $recruitment_today['subject_name'] . '-' . config('global_config.sitename'),
            "Description" => '最新招聘信息，点击查看 >>',
            "PicUrl" => $wechat_info_img,
            "Url" => $mobile_page
          ];
          $this->outputArticle($object, $content_arr);
        } else {
          $this->outputText($object, '今日招聘不存在或已删除');
        }
        break;
      case 'admin_bind':
      ...
      case 'subscribe_live':
        if (isset($event['scene_uuid'])) {
          $mobile_page = 'https://live.74cms.com/' . model('SceneQrcode')->type_arr['live']['mobile_page'] . '?' . model('SceneQrcode')->type_arr['live']['miniprogram_param'];
          $mobile_page = str_replace(":appkey", $event['appkey'], $mobile_page);
          $mobile_page = str_replace(":appsecret", $event['appsecret'], $mobile_page);
          $mobile_page = str_replace(":live_id", $event['live_id'], $mobile_page);
          // $mobile_page = 'http://live.74cms.com/m/#/?appkey=164L0q7lHP571k3A4G61&appsecret=V1W86N95f475U7Z13teum4tIa1&live=b85b419fe0a33a4f9b58e66efc095efd';
        }
        $content_arr = [
          "Title" => $event['share_title'],
          "Description" => $event['share_describe'] . '，点击进入直播间',
          "PicUrl" => $event['share_picture'],
          "Url" => $mobile_page
        ];
        $this->outputArticle($object, $content_arr);
        break;
