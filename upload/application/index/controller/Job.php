...
use app\common\lib\JobSearchEngine;
use Think\Model;
...
class Job extends \app\index\controller\Base {
  ...
  public function getData() {
    ...
    $mc = model('Category');
    $data = [
      'jobs' => model('CategoryJob')->field('id,name')->select(),
      'job' => input('param.job/d', null, 'intval'),

      'citys' => model('CategoryDistrict')->where('pid', cookie('cityId'))->select(),
      'city' => input('param.city/s', null, 'trim'),

      'jobtags' => $mc->where('alias', 'QS_jobtag')->select(),
      'trade' => $mc->where('alias', 'QS_trade')->select(),
      'company_type' => $mc->where('alias', 'QS_company_type')->select(),
      'education' => $mc->map_education,
      'experience' => $mc->map_experience,
      'wage' => [
        ['label' => '不限', 'min' => null, 'max' => null],
        ['label' => '2000以内', 'min' => 0, 'max' => 2000],
      ...
      'history' => model('History')->alias('a')
        ->where([
          'a.create' => $uid,
          'a.model' => 'Job'
        ])
        ->join(
          config('database.prefix') . 'category_job b',
          'a.mid=b.id',
          'LEFT'
        )->field('b.id,b.name')
    ...
    $cid = [];
    if (input('?param.company_type/d')) {
      $cid['nature'] = input('param.company_type/d', null, 'intval');
    }
    if (input('?param.trade/d')) {
      $cid['trade'] = input('param.trade/d', null, 'intval');
    }
    $where = [
      'company_id' => count($cid) ? ['in', model('Company')->where($cid)->column('id')] : null,
      'category' => $data['job'],
      'district' => $data['city'],
      'addtime' => input('?param.time') ?
        ['BETWEEN', input('param.time/s', null, 'trim')] : null,
      'tag' => input('?param.jobtag') ?
        ['like', '%' . input('param.jobtag/s', null, 'trim') . '%'] : null,
      'education' => input('param.education/d', null, 'trim'),
      'experience' => input('param.experience/d', null, 'trim'),
    ];
    model('History')->adddata([
      'create' => $uid,
      'model' => 'Job', 'mid' => $where['category']
    ]);
    try {
      $t = $data['wage'][input('param.wage/d', 0, 'intval')];
      $where['minwage'] = $t['min'] ? ['>=', $t['min']] : null;
      $where['maxwage'] = $t['max'] ? ['<=', $t['max']] : null;
    } catch (\Throwable $th) {
    }
    ...
    $total = $mj->where($where)->count();
    foreach ($where as $k => $v) {
      unset($where[$k]);
      $where['a.' . $k] = $v;
    }
    $data['data'] = [
      'items' => $mj->where($where)
        ->alias('a')
        ->join(
        ...
        ->page($page . ',' . $pagesize)
        ->field('a.*,a.`district` district_,b.name company_name,b.photo,c.last_login_time,' . $uid . ' apply_text')
        // ->fetchSql()
        ->select(),
      'total' => $total,
      'current_page' => $page,
  ...
  public function getJobs($param = null, $sum = 3) {
    if (!is_array($param)) {
      $param = [
        'count_total' => 1,
        'current_page' => 1,
    ...
    $instance = new JobSearchEngine($param);
    $searchResult = $instance->run();
    return $this->get_datalist($searchResult['items']);
  }
  ...
  public function show($a = null, $b = null, $c = null, $d = null) {
    $uid = $this->userinfo->uid ? $this->userinfo->uid : 0;
    $data = model('Job')->where('id', request()->route('id/d', 0, 'intval'))
      ->field(
        "id,uid,jobname,company_id company,id info5,$uid apply_text" .
          ',minwage,maxwage,negotiable,education,tag tag_,address,map_lat,map_lng'
          . ',map_zoom,refreshtime,addtime,amount,experience,content'
          . ',category1,category2,category3,district1,district2,district3,nature'
      )
      ->find();
    if (!$data) {
      abort(404, '页面不存在');
    }
    $data['wage_text'] = model('BaseModel')->handle_wage(
      $data['minwage'],
      $data['maxwage'],
      $data['negotiable']
    );
    $data['education_text'] = isset(
      model('BaseModel')->map_education[$data['education']]
    )
      ? model('BaseModel')->map_education[$data['education']]
      : '不限';
    $data['amount_text'] = $data['amount'] == 0 ? '若干' : $data['amount'];
    $data['experience_text'] = isset(
      model('BaseModel')->map_experience[$data['experience']]
    )
      ? model('BaseModel')->map_experience[$data['experience']]
      : '不限';
    $data['refreshtime'] = daterange_format(
      $data['addtime'],
      $data['refreshtime']
    );
    ...
    $similar_list = $instance->run('id != ' . $data['id']);
    $data['similar'] = $this->get_datalist($similar_list['items']);
  ...
  public function show2($a = null, $b = null, $c = null, $d = null) {
    ...
    $job = model('Job')->field('id,uid,id info5,' .
      ($this->userinfo->uid ? $this->userinfo->uid : 0)
      . ' apply_text')->where('id', $id)->find();
    if ($this->userinfo->uid) {
      $apply_text = $job['apply_text'];
      $this->assign('apply_text', $apply_text);
      $this->assign('companyid', $job['uid']);
    }
    // outp($return);
  ...
  protected function writeShowCache($id, $pageCache) {
    ...
    if ($companyinfo === null) {
      ...
      $return['com_contact'] = model('CompanyContact')->where('id', $companyinfo['id'])->find();
   ...
   * @author chenyang
   * @return array
   * Date Time：2022年4月28日17:15:29
   */
  public function register() {
    if (is_mobile_request() === true) {
      $this->redirect(config('global_config.mobile_domain') . 'jobRegister', 302);
      exit;
    }
    // 获取模板信息
    $where = [
    ...
    $templateModel = Model('form_template');
    $templateInfo = $templateModel->getInfo($where, 'template_id');
    if (empty($templateInfo)) {
      // 模板不存在时新增默认模板
      $formTemplateLogic = new \app\common\logic\FormTemplateLogic();
      $result = $formTemplateLogic->insertDefaultTemplate();
      if ($result['status'] === false) {
        abort(404, '新增默认模板失败，请联系网站管理员');
      }
      $templateInfo['template_id'] = $result['data'];
    }
    // 获取模板字段
    $where = [
    ...
    $order = ['a.sort' => 'asc'];
    $templateFieldModel = Model('form_template_field');
    $fieldList = $templateFieldModel->getJoinFieldList($where, $field, $order);
    if (empty($fieldList)) {
      abort(404, '模板字段不存在，请联系网站管理员');
    }
    // 获取字段内容
    $fieldIdArr = array_column($fieldList, 'field_id');
    // 获取字段内容
    $where = [
      'field_id' => ['in', $fieldIdArr]
    ];
    $fieldValueModel = Model('field_value');
    $fieldValueTemp = $fieldValueModel->getList($where, '*', ['sort' => 'asc']);
    $fieldValueList = [];
    if (!empty($fieldValueTemp)) {
      foreach ($fieldValueTemp as $fieldValueInfo) {
        $keys = 'field_id_' . $fieldValueInfo['field_id'];
        $fieldValueList[$keys][] = $fieldValueInfo;
      }
    }
    $fieldAliasArr = [];
    foreach ($fieldList as &$fieldInfo) {
      // 设置字段可输入长度
      $fieldLength = 0;
      if ($fieldInfo['field_type'] == 1 && $fieldInfo['field_way'] == 2) {
        // 手机号
        $fieldLength = 11;
      } elseif ($fieldInfo['field_type'] == 1) {
        // 文本
        $fieldLength = 20;
      } elseif ($fieldInfo['field_type'] == 5) {
        // 多行文本
        $fieldLength = 300;
      }
      $fieldInfo['field_length'] = $fieldLength;
      // 追加字段内容
      $keys = 'field_id_' . $fieldInfo['field_id'];
      if (isset($fieldValueList[$keys]) && !empty($fieldValueList[$keys])) {
        $fieldInfo['field_value'] = $fieldValueList[$keys];
      }
      #################### 返给前端form中的字段 ####################
      $fieldAliasArr[$fieldInfo['field_alias']] = '';
      // 多选框默认值要改为数组形式
      if ($fieldInfo['field_type'] == 3) {
        $fieldAliasArr[$fieldInfo['field_alias']] = [];
      }
      if ($fieldInfo['field_type'] == 1 && $fieldInfo['field_way'] == 2) {
        $fieldAliasArr['verify_code'] = '';
      }
    }

    $fieldAliasJson = json_encode($fieldAliasArr);
    $configData = config('global_config');

    $this->assign('list', $fieldList);
    $this->assign('list_json', json_encode($fieldList));
    $this->assign('field_alias_json', $fieldAliasJson);
    $this->assign('global_data', $configData);
    $pageHeader = $this->pageHeader;
    $pageHeader['title'] = '求职登记 - ' . $this->pageHeader['og']['title'];
