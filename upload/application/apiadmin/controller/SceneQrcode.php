...
class SceneQrcode extends \app\common\controller\Backend {
    ...
    public function add() {
        ...
        if ($input_data['platform'] == 2) {
            $class = new \app\common\lib\WechatMiniprogram;
            $param = str_replace(":id", $input_data['paramid'], $typeinfo['miniprogram_param']);
            $param = $param == '' ? ('scene_id=' . $insertid) : ($param . '&scene_id=' . $insertid);
            $qrcodeSrc = $class->makeQrcode($typeinfo['miniprogram_page'], $param, 'scene_qrcode_miniprogram_' . $insertid . '.jpg', false);
        } else if ($input_data['platform'] == 0) {
    ...
    public function searchJobfair($keyword) {
        if (!$keyword) {
            $return = [];
        } else {
            $list = model('Jobfair')
