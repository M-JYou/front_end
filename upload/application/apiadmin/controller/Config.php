...
class Config extends \app\common\controller\Backend {
  public function raw() {
    $mm = model('Config');
    if ($t = input('param.id/d', 0)) {
      $mm = $mm->where('id', $t);
    } elseif (($t = input('param.id/a', [])) && count($t)) {
      $mm = $mm->where('id', 'in', $t);
    }
    if ($t = input('param.name/s', '')) {
      $mm = $mm->where('name', 'like', $t);
    }
    if (($t = input('param.is_frontend/d')) !== null) {
      $mm = $mm->where('is_frontend', $t);
    }
    if ($t = input('param.value/s', '')) {
      $mm = $mm->where('value', 'like', $t);
    }
    if ($t = input('param.note/s', '')) {
      $mm = $mm->where('note', 'like', $t);
    }
    if (($t = input('param.is_secret/d')) !== null) {
      $mm = $mm->where('is_secret', $t);
    }

    $this->ajaxReturn(200, '获取数据成功', $mm->select());
  }
  public function index() {
    if (request()->isGet()) {
      if (input('get.#find')) {
        $this->_find(input('get.'));
      }
      ...
      $info['wechatMiniprogramQrcodeUrl'] = model('Uploadfile')->getFileUrl($info['wechat_miniprogram_qrcode']);
      ...
      $info['cityinfo_kefu_url'] = model('Uploadfile')->getFileUrl($info['cityinfo_kefu']);
      ...
      $info['live_app_secret_backups'] = $info['live_app_secret'];
      if (isset($info['live_app_key']) && $info['live_app_key'] != '') {
        $info['live_app_secret'] = md5(md5($info['live_app_key'] . $info['live_app_secret']) . 'ergFGsdfgf545');
      } else {
        $info['live_app_secret'] = '';
      }
      $this->ajaxReturn(200, '获取数据成功', $info);
    } else {
      $inputdata = input('post.');
      // $configlist = model('Config')->column('name,id');
      $configlist = model('Config')->column('id,name,value');
      $sqldata = [];
      $name_list = [];
          ...
          if ($c['name'] == $key) {
            ...
            if (is_array($value)) {
              ...
              if (is_string($value)) {
                ...
                if (is_file(SYS_UPLOAD_PATH . $value)) {
                  (new FileManager())->delete(model('Config')->where('value', 'like', '%' . $value)->column('id'));
                } elseif (is_file(SYS_ROOT_PATH . $value)) {
                  unlink(SYS_ROOT_PATH . $value);
                }
              }
            }
            $sqldata[] = $arr;
      ...
      model('Config')->isUpdate()->saveAll($sqldata);
