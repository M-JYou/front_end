...
class Chat extends \app\common\controller\Backend {
  protected $type = [
    'text',
    'img',
    'file',
  ...
  public function listUnread($json = 0) {
    $ret = model($this->dbname)
      ...
      ->where(['a.target' => 0, 'a.new' => 1])
      ->group('a.create')
      ->order('a.addtime desc')
      ->field('b.id,b.nickname,b.photo,b.online,(SELECT count(*) FROM `'
        . config('database.prefix')
        . 'chat` c WHERE c.`create`=a.`create` AND c.`target`=0 AND c.`new`=1) sum')
      // ->fetchSql()
      ->select();
    if ($json) {
      return $json;
  ...
  public function listAll($json = 0) {
    ...
    $d = model('MemberInfo')->alias('a')
      ...
      ->field('a.id,a.nickname,a.photo,a.online,(SELECT count(*) FROM `' . config('database.prefix')
        . 'chat` b WHERE b.`create`=a.id AND b.`target`=\'' . 0
        . '\' AND b.`new`=1) sum')
      ->select();
    $ret = [];
    foreach ($ids as $v) {
  ...
  public function chats() {
    $w = [
      'target&create' => ['in', [input('param.target/d', 0, 'intval'), 0]],
