...
class Jobfairol extends \app\common\controller\Backend {
    ...
    public function setStatus() {
        ...
        $company = model('Company')->whereIn('uid', $company)->column('uid,companyname', 'uid');
        $member = model('Resume')->whereIn('uid', $member)->column('uid,fullname', 'uid');
        foreach ($data as $k => $v) {
            $username = '';
            $type = '';
            if ($v['utype'] == 2) {
                $username = $member[$v['uid']];
                $type = '个人';
            } elseif ($v['utype'] == 1) {
                $username = $company[$v['uid']];
                $type = '企业';
            }
            $first = '网络招聘会审核通知';
            $keyword1 = '报名类型【' . $type . '报名】报名信息【' . $username . '】场次名称【网络招聘会名称】';
            $remark = '';
