...
class Member extends \app\common\controller\Backend {
  ...
  public function index() {
    ...
    if ($list_type == 'company') {
      ...
      $total = $total->join(config('database.prefix') . 'company b', 'a.uid=b.uid', 'LEFT')->join(config('database.prefix') . 'resume c', 'a.uid=c.uid', 'LEFT')->where(function ($query) {
    ...
    if ($list_type == 'company') {
      ...
      $list = $list->join(config('database.prefix') . 'company b', 'a.uid=b.uid', 'LEFT')->join(config('database.prefix') . 'resume c', 'a.uid=c.uid', 'LEFT')->where(function ($query) {
        $query->where('(b.companyname="" or b.companyname is NULL) AND a.utype=1')->whereOr('c.fullname IS NULL AND a.utype=2');
      });
    }
    $list = $list->field($field)->where($where)->where($wheres)->join(config('database.prefix') . 'member_bind d', 'd.uid=a.uid and d.type="weixin"', 'left')->order($order)->page($current_page . ',' . $pagesize)->select();
  ...
  public function add() {
    ...
    if (
      false ===
      model('Member')->validate('Member.add')->allowField(true)->save($input_data)
    ) {
  ...
  public function edit() {
    ...
    if ($uid) {
      ...
      if (
        false ===
        model('Member')->validate('Member.edit')->allowField(true)->save($input_data, ['uid' => $uid])
      ) {
        $this->ajaxReturn(500, model('Member')->getError());
      }
      model('AdminLog')->record(
  ...
  protected function points_list_company() {
    ...
    if ($keyword && $key_type) {
      switch ($key_type) {
        ...
        case 3:
          ...
          break;
        case 4:
          $map_uid_arr = model('Member')->where('mobile', 'like', '%' . $keyword . '%')->where(['utype' => ['eq', 1]])->column('uid');
    ...
    $total = model('Company')->alias('c')->join(
      config('database.prefix') . 'member_points p',
      'c.uid=p.uid',
      'LEFT'
    )->where($where)->count();
    $list = model('Company')->alias('c')->join(
      ...
      'c.uid=p.uid',
      'LEFT'
    )->field('c.uid,c.companyname,p.points,b.contact,b.mobile')->where($where)->order('c.uid desc')->page($current_page . ',' . $pagesize)->select();
    foreach ($list as $key => $value) {
  ...
  protected function points_list_personal() {
    ...
    $total = model('Member')->alias('m')->join(
      config('database.prefix') . 'member_points p',
      'm.uid=p.uid',
      'LEFT'
    )->join(
      ...
      'm.uid=r.uid',
      'LEFT'
    )->where($where)->where('r.id', 'not null')->count();
    $list = model('Member')->alias('m')->join(
      config('database.prefix') . 'member_points p',
      'm.uid=p.uid',
      'LEFT'
    )->join(
      ...
      'm.uid=r.uid',
      'LEFT'
    )->field('m.uid,m.mobile,m.reg_time,p.points,r.fullname')->where($where)->where('r.id', 'not null')->order('m.uid desc')->page($current_page . ',' . $pagesize)->select();
  ...
  public function points_set() {
    ...
    $charge_val = $is_charge === 1 ? input('post.charge_val/f', 0, 'floatval') : 0;
    $explain = input('post.explain/s', '', 'trim') . ('。收费' . $charge_val . '元');
    if ($uid == 0) {
    ...
    // model('AdminLog')->record(
    // '变更会员' . config('global_config.points_byname') . '。' . ($op == 1 ? '增加' : '减少') . '【' . $points_val . '】;会员UID【' . $uid . '】',
    // $this->admininfo
    // );
    $this->ajaxReturn(200, '操作成功');
  }
  public function points_log() {
    ...
    $list = model('MemberPointsLog')->where($where)->order('id desc')->page($current_page . ',' . $pagesize)->select();
    $return['items'] = $list;
    $return['total'] = $total;
    $return['current_page'] = $current_page;
    $return['pagesize'] = $pagesize;
  ...
  public function loginlog() {
    ...
    $total = model('MemberActionLog')->where($where)->count();
    $list = model('MemberActionLog')->where($where)->order('id desc')->page($current_page . ',' . $pagesize)->select();
    foreach ($list as $key => $value) {
  ...
  public function actionlog() {
    ...
    $total = model('MemberActionLog')->alias('a')->where($where)->where($wherefulltext)->count();
    ...
    $list = $list->where($where)->where($wherefulltext)->order('a.id desc')->page($current_page . ',' . $pagesize)->select();
    foreach ($list as $key => $value) {
      $list[$key]['platform_cn'] = isset(model('BaseModel')->map_platform[$value['platform']]) ? model('BaseModel')->map_platform[$value['platform']] : '-';
    }
    $return['items'] = $list;
  ...
  protected function managementExtra($uid) {
    ...
    $next_code = 200;
    if ($userinfo['utype'] == 1) {
      $company_profile = model('Company')->field(true)->where('uid', 'eq', $uid)->find();
      ...
      do {
        $resume_info = model('Resume')->field(true)->where('uid', 'eq', $uid)->find();
        if ($resume_info === null) {
          $next_code = 50007;
          break;
        }
        $intention = model('ResumeIntention')->field('id')->where('rid', $resume_info['id'])->find();
        if ($intention === null) {
          $next_code = 50005;
          break;
        }
  ...
  public function getUser() {
    $this->ajaxReturn(200, '获取数据成功', model('MemberInfo')->where('id', input('post.id/d'))->find());
  }
  public function setUserPoints() {
    $param = [
      'uid' => input('post.id/d', 0, 'intval'),
      'points' => input('post.points/d', 0, 'intval'),
      'note' => '【' . $this->admininfo->username . '】操作:' . input('post.note/s', '', 'trim'),
    ];
    $e = '操作成功';
    $code = 200;
    $d = null;
    try {
      model('Member')->setPoints([$param]);
      $d = model('MemberPoints')
        ->field('uid as id,points')
        ->where('uid', $param['uid'])
        ->find();
    } catch (\Throwable $th) {
      $e = $th->getMessage();
