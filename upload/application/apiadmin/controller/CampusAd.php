...
class CampusAd extends \app\common\controller\Backend {
    public function index() {
        ...
        $total = model('CampusAd')->alias('a')->join(config('database.prefix') . 'campus_ad_category b', 'a.cid=b.id', 'LEFT')
            ->where($where)
            ->count();
        $list = model('CampusAd')->alias('a')->field('a.*')->join(config('database.prefix') . 'campus_ad_category b', 'a.cid=b.id', 'LEFT')
            ->where($where)
            ->order('a.id asc')
        ...
        $category_arr = model('CampusAdCategory')->getCache();
        foreach ($list as $key => $value) {
    ...
    public function add() {
        ...
        if (
            ...
            $this->ajaxReturn(500, model('CampusAd')->getError());
        }
    ...
    public function edit() {
        ...
        if ($id) {
            $info = model('CampusAd')->find($id);
            if (!$info) {
                $this->ajaxReturn(500, '数据获取失败');
            }
            $info = $info->toArray();
            $ad_category = model('CampusAdCategory')
                ->where('id', $info['cid'])
                ->find();
    ...
    public function innerLinkOptions() {
        $list = model('CampusAd')->innerLinks;
