...
class CampusNotice extends \app\common\controller\Backend {
    public function index() {
        ...
        foreach ($list as $key => $value) {
            if ($value['link_url'] == '') {
                $value['link'] = url('campus/notice/' . $value['id']);
            } else {
    ...
    public function add() {
        ...
        if ($input_data['holddate_start']) {
            $input_data['holddate_start'] = strtotime($input_data['holddate_start']);
        }
        if ($input_data['holddate_end']) {
            $input_data['holddate_end'] = strtotime($input_data['holddate_end']);
        }
        if (
            false ===
            model('CampusNotice')
            ...
            $this->ajaxReturn(500, model('CampusNotice')->getError());
        }

        model('AdminLog')->record(
            '添加公告。公告ID【' .
                model('CampusNotice')->id .
                '】;公告标题【' .
                $input_data['title'] .
                '】',
            $this->admininfo
    ...
    public function edit() {
        ...
        if ($id) {
            $info = model('CampusNotice')->find($id);
            if (!$info) {
