...
class Captcha {
  ...
  public function checkWithJwt($code, $secret_str, $id = '') {
    try {
      $token = \app\common\lib\JwtAuth::getToken($secret_str);
    } catch (\Exception $e) {
      return false;
    }
    if ($token->isExpired()) {
    ...
    $info = $token->getData('info');
    if ($info !== $this->authcode(strtoupper($code))) {
      return false;
    }
    return true;
  }
   ...
   */
  public function entryWithJwt($id = '') {
    // 图片宽(px)
    $this->imageW ||
      ($this->imageW =
        $this->length * $this->fontSize * 1.5 +
    ...
    $ttfPath =
      __DIR__ . '/../assets/' . ($this->useZh ? 'zhttfs' : 'ttfs') . '/';
    ...
    if ($this->useArithmetic) {
      ...
      if ($this->useZh) {
        ...
        for ($i = 0; $i < $this->length; $i++) {
          $code[$i] =
            $this->codeSet[mt_rand(0, strlen($this->codeSet) - 1)];
          $codeNX += mt_rand(
            $this->fontSize * 1.2,
            $this->fontSize * 1.6
          );
      ...
      if (\think\Config::get('app_debug')) {
        $retCode = strtoupper(implode('', $code));
      }
    ...
    for ($i = 0; $i < 10; $i++) {
      ...
      for ($j = 0; $j < 5; $j++) {
        ...
        imagestring(
          ...
          $codeSet[mt_rand(0, strlen($codeSet) - 1)],
          $noiseColor
        );
      }
    }
